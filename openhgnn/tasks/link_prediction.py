...
class LinkPrediction(BaseTask):
    ...
    def __init__(self, args):
        ...
        self.dataset = build_dataset(
            args.dataset, "link_prediction", logger=self.logger
        )
        ...
        self.evaluation_metric = getattr(
            args, "evaluation_metric", "roc_auc"
        )  # default evaluation_metric is roc_auc
        ...
        args.logger.info(
            "[Init Task] The task: link prediction, the dataset: {}, the evaluation metric is {}, "
            "the score function: {} ".format(
                self.name_dataset, self.evaluation_metric, args.score_fn
            )
        )
    ...
    def evaluate(self, n_embedding, r_embedding=None, mode="test"):
        ...
        if self.evaluation_metric == "acc":
            ...
            mrr_matrix = self.evaluator.mrr_(
                n_embedding,
                r_embedding,
                self.dataset.train_triplets,
                self.dataset.valid_triplets,
                self.dataset.test_triplets,
                score_predictor=self.ScorePredictor,
                hits=[1, 3, 10],
                filtered=getattr(self, "filtered", "filtered"),
                eval_mode=mode,
            )
            return mrr_matrix
            ...
            roc_auc = self.evaluator.cal_roc_auc(
                th.cat((p_label, n_label)).cpu(), th.cat((p_score, n_score)).cpu()
            )
    ...
    def tranX_predict(self):
        ...
        score = th.sigmoid(
            self.ScorePredictor(pred_triples_T[0], pred_triples_T[1], pred_triples_T[2])
        )
        indices = self.pred_hg.edges()
        return indices, score
    ...
    def downstream_evaluate(self, logits, evaluation_metric):
        if evaluation_metric == "academic_lp":
            auc, macro_f1, micro_f1 = self.evaluator.author_link_prediction(
                logits, self.dataset.train_batch, self.dataset.test_batch
            )
