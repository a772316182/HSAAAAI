...
class NodeClassification(BaseTask):
    ...
    def __init__(self, args):
        ...
        self.dataset = build_dataset(
            args.dataset, "node_classification", logger=self.logger
        )
...
class DSSL_task(BaseTask):
    ...
    def get_rand_split(self):
        self.train_idx, self.val_idx, self.test_idx = self.get_idx_split(
            train_prop=self.train_prop, valid_prop=self.valid_prop
        )
        return self.train_idx, self.val_idx, self.test_idx
    ...
    def rand_train_test_idx(
        self, label, train_prop=0.5, valid_prop=0.25, ignore_negative=True
    ):
