...
class SkipGramModel(nn.Module):
    ...
    def fast_learn(self, batch_walks, neg_nodes=None):
        ...
        if self.use_context_weight:
            if bs < self.batch_size:
                context_weight = init_weight(self.walk_length, self.window_size, bs).to(
                    self.device
                )
            else:
        ...
        if neg_nodes is not None:
            grad_v_neg = adam(
                grad_v_neg, self.state_sum_v, neg_nodes, lr, self.device, self.only_gpu
            )
    ...
    def save_embedding_pt(self, dataset, file_name):
        ...
        try:
            ...
            index = torch.LongTensor(
                list(map(lambda id: dataset.id2node[id], list(range(self.emb_size))))
            )
