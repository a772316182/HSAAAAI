...
class DiffMG(BaseModel):
    ...
    def __init__(
        ...
        for i in range(len(n_steps)):
            self.metas.append(
                Cell(n_steps[i], n_hid, n_hid, use_norm=use_norm, use_nl=out_nl)
            )
...
class Cell1(nn.Module):
    ...
    def __init__(self, n_step, n_hid_prev, n_hid, cstr, use_norm=True, use_nl=True):
        ...
        self.norm = (
            nn.LayerNorm(n_hid, elementwise_affine=False)
            if use_norm is True
            else lambda x: x
        )
        self.use_nl = use_nl
    ...
    def forward(self, x, adjs, ws_seq, idxes_seq, ws_res, idxes_res):
        ...
        for i in range(self.n_step - 1):
            seqi = self.ops_seq[i](
                states[i], adjs[:-1], ws_seq[0][i], idxes_seq[0][i]
            )  # ! exclude zero Op
            resi = sum(
                self.ops_res[offset + j](
                    h, adjs, ws_res[0][offset + j], idxes_res[0][offset + j]
                )
                for j, h in enumerate(states[:i])
            )
            offset += i
        ...
        out_res = sum(
            self.last_res[i](h, adjs_cstr, ws_res[1][i], idxes_res[1][i])
...
class search_model(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(n_steps)):
            self.metas.append(
                Cell1(n_steps[i], n_hid, n_hid, cstr, use_norm=use_norm, use_nl=out_nl)
            )
        ...
        ks = [
            sum(1 for i in range(2, n_steps[k]) for j in range(i - 1))
        ...
        assert (
            ks[0]
            + n_steps[0]
            + (0 if self.as_last_res[0] is None else self.as_last_res[0].size(0))
    ...
    def sample(self, eps):
        ...
        if np.random.uniform() < eps:
            for i in range(len(self.metas)):
                ...
                temp.append(
                    None
                    if self.as_seq[i] is None
                    else torch.randint(
                        low=0,
                        high=self.as_seq[i].size(-1),
                        size=self.as_seq[i].size()[:-1],
                    )
                )
                temp.append(
                    torch.randint(low=0, high=self.as_last_seq[i].size(-1), size=(1,))
                )
            ...
            for i in range(len(self.metas)):
                ...
                temp.append(
                    None
                    if self.as_res[i] is None
                    else torch.randint(
                        low=0,
                        high=self.as_res[i].size(-1),
                        size=self.as_res[i].size()[:-1],
                    )
                )
                temp.append(
                    None
                    if self.as_last_res[i] is None
                    else torch.randint(
                        low=0,
                        high=self.as_last_res[i].size(-1),
            ...
            for i in range(len(self.metas)):
                ...
                temp.append(
                    None
                    if self.as_seq[i] is None
                    else torch.argmax(F.softmax(self.as_seq[i], dim=-1), dim=-1)
                )
                temp.append(
                    torch.argmax(F.softmax(self.as_last_seq[i], dim=-1), dim=-1)
                )
                idxes_seq.append(temp)
            for i in range(len(self.metas)):
                temp = []
                temp.append(
                    None
                    if self.as_res[i] is None
                    else torch.argmax(F.softmax(self.as_res[i], dim=-1), dim=-1)
                )
                temp.append(
                    None
                    if self.as_last_res[i] is None
                    else torch.argmax(F.softmax(self.as_last_res[i], dim=-1), dim=-1)
                )
                idxes_res.append(temp)
    ...
    def forward(self, node_feats, node_types, adjs, idxes_seq, idxes_res):
        ...
        for i, meta in enumerate(self.metas):
            ws_seq = []
            ws_seq.append(
                None if self.as_seq[i] is None else F.softmax(self.as_seq[i], dim=-1)
            )
            ws_seq.append(F.softmax(self.as_last_seq[i], dim=-1))
            ws_res = []
            ws_res.append(
                None if self.as_res[i] is None else F.softmax(self.as_res[i], dim=-1)
            )
            ws_res.append(
                None
                if self.as_last_res[i] is None
                else F.softmax(self.as_last_res[i], dim=-1)
            )
    ...
    def parse(self):
        ...
        for i in range(len(idxes_seq)):
            ...
            msg_seq.append(
                map_seq
                if idxes_seq[i][0] is None
                else idxes_seq[i][0].tolist() + map_seq
            )
            ...
            assert (
                len(temp_res) == self.metas[i].n_step * (self.metas[i].n_step - 1) // 2
            )
