...
class RSHN(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        ...
        linear_e1 = nn.Linear(
            in_features=cl_graph.num_nodes(), out_features=args.hidden_dim, bias=False
        )
    ...
    def __init__(self, dim, out_dim, num_node_layer, num_edge_layer, dropout):
        ...
        for i in range(num_node_layer):
            self.node_layers.append(
                GraphConv(
                    in_feats=dim, out_feats=dim, dropout=dropout, activation=th.tanh
                )
            )
