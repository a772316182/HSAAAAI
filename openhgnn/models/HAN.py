...
class HAN(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        ...
        for ntype in ntypes:
            ...
            if args.meta_paths_dict is None:
                args.meta_paths_dict = extract_metapaths(
                    args.category, hg.canonical_etypes
                )
    ...
    def __init__(
        self, ntype_meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout
    ):
        super(HAN, self).__init__()
        self.out_dim = out_dim
        self.mod_dict = nn.ModuleDict()
        print("metapath", ntype_meta_paths_dict)
        ...
        for ntype, meta_paths_dict in ntype_meta_paths_dict.items():
            print(ntype, meta_paths_dict)
            self.mod_dict[ntype] = _HAN(
                meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout
            )
...
class _HAN(nn.Module):
    ...
    def __init__(
        self, meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout
    ):
        super(_HAN, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(
            HANLayer(meta_paths_dict, in_dim, hidden_dim, num_heads[0], dropout)
        )
        ...
        # full batch
            ...
            if self._cached_graph is None or self._cached_graph is not g:
                ...
                for mp, mp_value in self.meta_paths_dict.items():
                    if isinstance(mp_value[0], list):
                        mp_value = [tuple(item) for item in mp_value]
