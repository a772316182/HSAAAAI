...
class MLP(nn.Module):
    def __init__(
        self, in_channels, hidden_channels, out_channels, num_layers, dropout=0.0
    ):
...
class DSSL(BaseModel):
    ...
    def __init__(
        ...
        self.clusters = nn.Parameter(
            nn.init.normal_(torch.Tensor(hidden_channels, cluster_num))
        )
    ...
    def forward(self, embedding, neighbor_embedding):
        ...
        if self.inference_mlp == True:  # self.inference_mlp = True
            k, k_node, entropy_loss = self.inference_network(
                embedding_expand, neighbor_embedding
            )
        # else:
    ...
    def generative_network(self, embedding_expand, k, neighbor_embedding):
        ...
        central = self.mlp_predictor(
            embedding_expand
        ) + self.beta * self.mlp_predictor2(gumbel_k)
