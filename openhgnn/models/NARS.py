...
class NARS(BaseModel):
    ...
    def __init__(self, num_hops, args, hg):
        ...
        with th.no_grad():
            self.feats = preprocess_features(
                hg, mps, args, args.device, self.args.category
            )
...
def preprocess_features(g, mps, args, device, predict):
    ...
    for ntype in g.ntypes:
        ...
        if category_dim != ntype_dim:
            rand_weight = (
                th.Tensor(ntype_dim, category_dim).uniform_(-0.5, 0.5).to(device)
            )
            g.nodes[ntype].data["feat"] = th.matmul(
                g.nodes[ntype].data["feat"], rand_weight
            )

    num_paper, feat_size = g.nodes[predict].data["feat"].shape

    new_feats = [
        th.zeros(num_paper, len(mps), feat_size) for _ in range(args.num_hops + 1)
    ]
    ...
    for hop in range(1, args.num_hops + 1):
        ...
        for etype in new_g.etypes:
            ...
            new_g[etype].update_all(
                fn.copy_u(f"hop_{hop - 1}", "m"), fn.sum("m", "new_feat")
            )
