...
class KGCN(BaseModel):
    ...
    def __init__(self, g, args):
        ...
        self.entity_emb_matrix = nn.Parameter(
            th.FloatTensor(self.g.num_nodes(), self.in_dim)
        )
        self.relation_emb_matrix = nn.Parameter(
            th.FloatTensor(args.n_relation, self.in_dim)
        )
    ...
    def forward(self, blocks, inputdata):
        ...
        self.item_embeddings, self.userList, self.labelList = self.Aggregate(
            blocks, inputdata
        )
        self.get_score()
    ...
    def aggregate(self):
        ...
        for i in range(len(self.data)):
            ...
            if self.args.aggregate == "SUM":
                embeddingList.append(
                    self.sub_g.dstdata["embedding"][i]
                    + self.sub_g.dstdata["ft"][i][weightIndex]
                )
                ...
                embeddingList.append(
                    th.cat(
                        [
                            self.sub_g.dstdata["embedding"][i],
                            self.sub_g.dstdata["ft"][i][weightIndex].squeeze(0),
                        ],
                        dim=-1,
                    )
                )
