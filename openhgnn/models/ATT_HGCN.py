...
class ATT_HGCN(BaseModel):
    ...
    def __init__(
        self, net_schema, layer_shape, label_keys, type_fusion="att", type_att_size=64
    ):
        super(ATT_HGCN, self).__init__()
        self.hgc1 = HeteGCNLayer(
            net_schema, layer_shape[0], layer_shape[1], type_fusion, type_att_size
        )
        self.hgc2 = HeteGCNLayer(
            net_schema, layer_shape[1], layer_shape[2], type_fusion, type_att_size
        )
        ...
        for k in label_keys:
            self.embd2class[k] = nn.Parameter(
                torch.FloatTensor(layer_shape[-2][k], layer_shape[-1][k])
            )
...
class HeteGCNLayer(nn.Module):
    ...
    def __init__(
        self, net_schema, in_layer_shape, out_layer_shape, type_fusion, type_att_size
    ):
        ...
        for k in net_schema:
            self.hete_agg[k] = HeteAggregateLayer(
                k,
                net_schema[k],
                in_layer_shape,
                out_layer_shape[k],
...
class HeteAggregateLayer(nn.Module):
    ...
    def __init__(
        self, curr_k, nb_list, in_layer_shape, out_shape, type_fusion, type_att_size
    ):
        ...
        for k in nb_list:
            try:
                self.W_rel[k] = nn.Parameter(
                    torch.FloatTensor(in_layer_shape[k], out_shape)
                )
            except KeyError as ke:
                self.W_rel[k] = nn.Parameter(
                    torch.FloatTensor(in_layer_shape[self.curr_k], out_shape)
                )
            finally:
    ...
    def forward(self, x_dict, adj_dict):
        ...
        if self.type_fusion == "mean":
            ...
            agg_nb_ft = (
                torch.cat([nb_ft.unsqueeze(1) for nb_ft in nb_ft_list], 1)
                .mul(attention.unsqueeze(-1))
                .sum(1)
            )
