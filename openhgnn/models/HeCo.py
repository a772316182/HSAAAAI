...
class HeCo(BaseModel):
    ...
    **Authors:** Xiao Wang, Nian Liu, Hui Han, Chuan Shi
    ...
    def __init__(
        ...
        self.sc = Sc_encoder(
            network_schema, hidden_size, self.attn_drop, sample_rate, self.category
        )
...
class Mp_encoder(nn.Module):
    def __init__(self, meta_paths_dict, hidden_size, attn_drop):
        ...
        for mp in meta_paths_dict:
            one_layer = GraphConv(
                hidden_size, hidden_size, activation=self.act, allow_zero_in_degree=True
            )
...
class Sc_encoder(nn.Module):
    ...
    def forward(self, g, h):
        ...
        for i, network_schema in enumerate(self.network_schema):
            ...
            sub_graph = sample_neighbors(
                one_graph,
                {self.category: cate_num},
                {network_schema[1]: self.sample_rate[src_type]},
                replace=True,
            )
