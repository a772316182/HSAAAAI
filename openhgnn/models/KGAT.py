...
class KGAT(BaseModel):
    ...
    def __init__(self, args):
        ...
        if args.aggregation_type not in ["bi-interaction", "gcn", "graphsage"]:
            raise KeyError(
                "Aggregator type {} not supported.".format(args.aggregation_type)
            )
    ...
    def set_parameters(
        self, n_users, n_entities, n_relations, user_pre_embed=None, item_pre_embed=None
    ):
        ...
        self.entity_user_embed = nn.Embedding(
            self.n_entities + self.n_users, self.entity_dim
        )
        if (
            (self.use_pretrain == 1)
            ...
            other_entity_embed = nn.Parameter(
                torch.Tensor(self.n_entities - item_pre_embed.shape[0], self.entity_dim)
            )
            nn.init.xavier_uniform_(
                other_entity_embed, gain=nn.init.calculate_gain("relu")
            )
        ...
        self.W_R = nn.Parameter(
            torch.Tensor(self.n_relations, self.entity_dim, self.relation_dim)
        )
    ...
    def att_score(self, edges):
        # Equation (4)
        r_mul_t = torch.matmul(
            self.entity_user_embed(edges.src["id"]), self.W_r
        )  # (n_edge, relation_dim)
        r_mul_h = torch.matmul(
            self.entity_user_embed(edges.dst["id"]), self.W_r
        )  # (n_edge, relation_dim)
        r_embed = self.relation_embed(edges.data["type"])  # (1, relation_dim)
        att = torch.bmm(
            r_mul_t.unsqueeze(1), torch.tanh(r_mul_h + r_embed).unsqueeze(2)
        ).squeeze(
            -1
        )  # (n_edge, 1)
        return {"att": att}
...
class KGAT_Aggregator(nn.Module):
    ...
    def forward(self, mode, g, entity_embed):
        ...
        if mode == "predict":
            g.update_all(
                dgl.function.u_mul_e("node", "att", "side"),
        ...
        if self.aggregator_type == "gcn":
            # Equation (6) & (9)
            out = self.activation(
                self.W(g.ndata["node"] + g.ndata["N_h"])
            )  # (n_users + n_entities, out_dim)

        elif self.aggregator_type == "graphsage":
            # Equation (7) & (9)
            out = self.activation(
                self.W(torch.cat([g.ndata["node"], g.ndata["N_h"]], dim=1))
            )  # (n_users + n_entities, out_dim)

        elif self.aggregator_type == "bi-interaction":
            # Equation (8) & (9)
            out1 = self.activation(
                self.W1(g.ndata["node"] + g.ndata["N_h"])
            )  # (n_users + n_entities, out_dim)
            out2 = self.activation(
                self.W2(g.ndata["node"] * g.ndata["N_h"])
            )  # (n_users + n_entities, out_dim)
            out = out1 + out2
        else:
            raise NotImplementedError
