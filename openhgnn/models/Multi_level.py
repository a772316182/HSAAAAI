...
class HGConvLayer(nn.Module):
    def __init__(
        ...
        if self.residual:
            ...
            for ntype in graph.ntypes:
                self.res_fc[ntype] = nn.Linear(
                    input_dim, n_heads * hidden_dim, bias=True
                )
    ...
    def reset_parameters(self):
        ...
        for weight in self.edge_type_transformation_weight:
            nn.init.xavier_normal_(
                self.edge_type_transformation_weight[weight], gain=gain
            )
        for weight in self.central_node_transformation_weight:
            nn.init.xavier_normal_(
                self.central_node_transformation_weight[weight], gain=gain
            )
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, node_features: dict):
        ...
        output_features = self.macro_conv(
            graph,
            input_dst,
            relation_features,
        ...
        if self.residual:
            for ntype in output_features:
                ...
                output_features[ntype] = output_features[ntype] * alpha + self.res_fc[
                    ntype
                ](input_dst[ntype]) * (1 - alpha)
