...
class HGCL(BaseModel):
    def build_model_from_args(args, hg):
        ...
        uiMat = hg.adj_external(
            etype=("user+item", "distance", "user+item"), scipy_fmt="csr"
        )
    ...
    def metaregular(self, em0, em, adj):
        def row_column_shuffle(embedding):
            ...
            corrupted_embedding = corrupted_embedding[
                torch.randperm(embedding.shape[0])
            ]
    ...
    def self_gatingu(self, em):
        return torch.multiply(
            em,
            torch.sigmoid(torch.matmul(em, self.gating_weightu) + self.gating_weightub),
        )

    def self_gatingi(self, em):
        return torch.multiply(
            em,
            torch.sigmoid(torch.matmul(em, self.gating_weighti) + self.gating_weightib),
        )
    ...
    def metafortansform(self, auxiembedu, targetembedu, auxiembedi, targetembedi):
        ...
        tembedu = self.meta_netu(
            t.cat((auxiembedu, targetembedu, uneighbor), dim=1).detach()
        )
        tembedi = self.meta_neti(
            t.cat((auxiembedi, targetembedi, ineighbor), dim=1).detach()
        )
        ...
        tembedus = t.sum(
            t.multiply((auxiembedu).unsqueeze(-1), low_weightu1), dim=1
        )  # Equal to a two-layer linear network; Ciao and Yelp data sets are plus gelu activation function
    ...
    def forward(self, iftraining, uid, iid, norm=1):
        ...
        ui_index = np.array(
            user_index.tolist() + [i + self.userNum for i in item_index]
        )
        ...
        for i in range(len(self.encoder)):
            ...
            self.ui_userEmbedding0, self.ui_itemEmbedding0 = t.split(
                uiEmbeddings0, [self.userNum, self.itemNum]
            )
        ...
        self.ui_userEmbedding, self.ui_itemEmbedding = t.split(
            self.uiEmbedding, [self.userNum, self.itemNum]
        )
        ...
        return (
            ...
            (self.wi1 * self.ui_itemEmbedding + self.wi2 * self.itemEmbedding),
            self.ui_userEmbedding,
            self.ui_itemEmbedding,
            metaregloss,
        )
