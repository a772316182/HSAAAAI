...
class NSHE(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        return cls(
            hg,
            "GCN",
            project_dim=args.dim_size["project"],
            emd_dim=args.dim_size["emd"],
            context_dim=args.dim_size["context"],
        )
    ...
    def __init__(self, g, gnn_model, project_dim, emd_dim, context_dim):
        ...
        self.feature_proj = HeteroLinearLayer(
            linear_dict1, has_l2norm=False, has_bn=False
        )
        # * ================== Neighborhood Agg(gnn_model)==================
        if self.gnn_model == "GCN":
            self.gnn1 = GraphConv(
                self.project_dim, self.emd_dim, norm="none", activation=F.relu
            )
            self.gnn2 = GraphConv(
                self.emd_dim, self.emd_dim, norm="none", activation=None
            )
        ...
        self.context_encoder = HeteroLinearLayer(
            linear_dict2, has_l2norm=False, has_bn=False
        )
        # * ================== NSI Classification================
        self.linear_classifier = HeteroMLPLayer(
            linear_dict3, has_l2norm=False, has_bn=False
        )
