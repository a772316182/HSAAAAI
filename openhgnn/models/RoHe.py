        ...
        for ntype, meta_paths_dict in ntype_meta_paths_dict.items():
            if len(meta_paths_dict) == 0:
                ntype_meta_paths_dict[ntype] = extract_metapaths(
                    ntype, hg.canonical_etypes
                )

        return cls(
            ntype_meta_paths_dict=ntype_meta_paths_dict,
...
class _HAN(nn.Module):
    ...
    def __init__(
        self, meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout, settings
    ):
        super(_HAN, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(
            HANLayer(
                meta_paths_dict, in_dim, hidden_dim, num_heads[0], dropout, settings
            )
        )
...
class HANLayer(nn.Module):
    ...
    def __init__(
        self, meta_paths_dict, in_dim, out_dim, layer_num_heads, dropout, settings
    ):
        ...
        mods = nn.ModuleDict(
            {
                ...
                for i, mp in enumerate(meta_paths_dict.keys())
            }
        )
...
class RoHeGATConv(nn.Module):
    ...
    def mask(self, attM):
        T = self.settings["T"]
        indices_to_remove = attM < torch.clamp(
            torch.topk(attM, T)[0][..., -1, None], min=0
        )
    ...
    def forward(self, graph, feat):
        ...
        e = torch.cat(
            [
                torch.matmul(
                    feat_src[:, i, :].view(N, self._out_feats),
        ...
        total_edge = torch.cat(
            (graph.edges()[0].view(1, N_e), graph.edges()[1].view(1, N_e)), 0
        )
        # confidence score in Eq(7)   change here device
        attn = torch.sparse.FloatTensor(
            total_edge.to(self.settings["device"]),
            torch.squeeze(
                (
                    e.to(self.settings["device"]) * e_trans.to(self.settings["device"])
                ).sum(-2)
            ),
            torch.Size([N, N]),
        ...
        e[
            attn[graph.edges()[0], graph.edges()[1]]
            .view(N_e, 1)
            .repeat(1, 8)
            .view(N_e, 8, 1)
            < -100
