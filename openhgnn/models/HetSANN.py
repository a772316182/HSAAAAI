        ...
        if hasattr(hg, "ntypes"):
            with hg.local_scope():
                ...
                for i in range(self.num_layers - 1):
                    h = self.het_layers[i](
                        g, h, g.ndata["_TYPE"], g.edata["_TYPE"], True
                    )
            ...
            h_dict = to_hetero_feat(
                h, block.ndata["_TYPE"]["_N"][: block.num_dst_nodes()], self.ntypes
            )
...
class HetSANNConv(nn.Module):
    ...
    def __init__(
        ...
        self.a_l = TypedLinear(
            self.hidden_dim * self.num_heads,
            self.hidden_dim * self.num_heads,
            num_etypes,
        )
        self.a_r = TypedLinear(
            self.hidden_dim * self.num_heads,
            self.hidden_dim * self.num_heads,
            num_etypes,
        )
    ...
    def forward(self, g, x, ntype, etype, presorted=False):
        ...
        # formula (2) (3) (4)
        h_l = (
            self.a_l(h.view(-1, self.num_heads * self.hidden_dim), etype, presorted)
            .view(-1, self.num_heads, self.hidden_dim)
            .sum(dim=-1)
        )

        h_r = (
            self.a_r(h.view(-1, self.num_heads * self.hidden_dim), etype, presorted)
            .view(-1, self.num_heads, self.hidden_dim)
            .sum(dim=-1)
        )
