    ...
    **Authors:** Chanyoung Park, Donghyun Kim, Jiawei Han, Hwanjo Yu
    ...
    def __init__(
        ...
        if self.isAttn:
            self.attn = nn.ModuleList(
                Attention(
                    hid_units=hidden_dim, num_mps=len(meta_paths), num_ndoes=num_nodes
                )
        ...
        self.init_weight()
        print(
            "category:{}, category's classes:{}, isBias:{},"
            " isAttn:{}, isSemi:{}".format(
                category, num_classes, isBias, isAttn, isSemi
            )
        )
    ...
    def forward(self, hg, samp_bias1=None, samp_bias2=None):
        ...
        for idx, meta_path in enumerate(self.meta_paths):
            ...
            shuf_feats[idx] = F.dropout(
                shuf_feats[idx], self.dropout, training=self.training
            )
...
class Attention(nn.Module):
    ...
    def attn_feature(self, features):
        ...
        features = (
            features.view(self.num_mps, self.num_nodes, self.hid_units)
            .sum(0)
            .unsqueeze(0)
        )
