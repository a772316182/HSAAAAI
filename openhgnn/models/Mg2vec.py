...
class Mg2vec(BaseModel):
    ...
    def __init__(self, node_num, mg_num, emb_dimension, unigram, sample_num):
        ...
        self.n_w_t = nn.Parameter(
            torch.empty([emb_dimension * 2, emb_dimension]), requires_grad=True
        )
    ...
    def forward(self, train_a, train_b, train_labels, train_freq, train_weight, device):
        ...
        mask_o1 = (
            torch.eq(train_a, train_b)
            .type(torch.FloatTensor)
            .reshape(batch_size, 1)
            .to(device)
        )
        mask_o2 = (
            torch.not_equal(train_a, train_b)
            .type(torch.FloatTensor)
            .reshape(batch_size, 1)
            .to(device)
        )
        m_embed_pos = self.m_embedding(train_labels)
        neg_sample_id = torch.multinomial(
            self.mg_unigrams, min(self.mg_num, self.sample_num)
        ).to(device)
        ...
        labels = torch.cat(
            [torch.ones_like(true_logit), torch.zeros_like(neg_logit)], dim=1
        )
        xent = torch.sum(
            F.binary_cross_entropy_with_logits(logit, labels, reduction="none"),
            dim=1,
            keepdim=True,
        )
    ...
    def normalize_embedding(self):
        norm = torch.sqrt_(
            torch.sum(torch.square(self.n_embedding.weight.data), dim=1, keepdim=True)
        )
        self.n_embedding.weight.data = self.n_embedding.weight.data / norm

        m_norm = torch.sqrt_(
            torch.sum(torch.square(self.m_embedding.weight.data), dim=1, keepdim=True)
        )
        ...
        with open(file_name, "w") as f:
            for nId, node in id2node.items():
                to_write = (
                    str(node)
                    + " "
                    + " ".join(map(lambda x: str(x), embedding[nId]))
                    + "\n"
                )
                f.write(to_write)
