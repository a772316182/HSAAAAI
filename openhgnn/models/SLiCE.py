...
class ScaledDotProductAttention(torch.nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        scores.masked_fill_(
            attn_mask == True, -1e9
        )  # change dropped softmax value into
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        )  # 128(batcch)*4(head)*7(n_nodes)*64(d_k)
        k_s = self.W_K(K).view(batch_size, -1, self.n_heads, self.d_k).transpose(1, 2)
        v_s = self.W_V(V).view(batch_size, -1, self.n_heads, self.d_v).transpose(1, 2)

        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).repeat(1, self.n_heads, 1, 1)
        context, attn = self.scaled_dot_prod_attn(
...
class SLiCE(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        ...
        return cls(
            G=hg, pretrained_node_embedding_tensor=None, args=args
        )  # to-do: 命令行解析
    ...
    def __init__(
        ...
        if not os.path.exists(args.pretrained_embeddings):
            ...
            for _ in range(10):
                ...
                walk = dgl.sampling.node2vec_random_walk(
                    G, torch.tensor(nodes), 1, 1, walk_length=80 - 1
                ).tolist()  # len=walk_length+1
                walks.extend(walk)
    ...
    def forward(self, subgraph_list):
        ...
        if self.fine_tuning_layer:
            masked_nodes = Variable(
                torch.LongTensor([[] for ii in range(len(subgraph_list))])
            )
        else:
        ...
        enc_self_attn_mask = get_attn_pad_mask(
            subgraph_list, self.no_nodes, self.max_length + 1
        )
        ...
        for layer in self.layers:
            ...
            try:
                layer_output = torch.cat(
                    (layer_output, output.unsqueeze(1)), 1
                )  # output embedding of each layer
            ...
            if self.fine_tuning_layer:
                try:
                    att_output = torch.cat(
                        (att_output, enc_self_attn.unsqueeze(0)), 0
                    )  # output attention of each layer
                except NameError:  # FIXME - replaced bare except
...
class SLiCEFinetuneLayer(torch.nn.Module):
    ...
    def build_model_from_args(cls, args):
        return cls(
            ...
            ft_d_ff=args.ft_d_ff,
            ft_layer=args.ft_layer,
            ft_drop_rate=args.ft_drop_rate,
            ft_input_option=args.ft_input_option,
