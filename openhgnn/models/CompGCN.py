...
class CompGCN(BaseModel):
    ...
    def __init__(
        ...
        self.layers.append(
            CompGraphConvLayer(
                self.hid_dim, self.out_dim, self.rel_names, comp_fn=self.comp_fn
            )
        )
...
class CompGraphConvLayer(nn.Module):
    ...
    def __init__(
        ...
        self.conv = dglnn.HeteroGraphConv(
            {
                rel: CompConv(comp_fn=comp_fn, norm="right", _allow_zero_in_degree=True)
    ...
    def forward(self, hg, n_in_feats, r_feats):
        ...
        with hg.local_scope():
            ...
            if hg.is_block:
                ...
                inputs_dst = {
                    k: v[: hg.number_of_dst_nodes(k)] for k, v in n_in_feats.items()
                }
