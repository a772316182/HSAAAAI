...
class ieHGCN(BaseModel):
    ...
    def __init__(
        ...
        self.hgcn_layers.append(
            ieHGCNConv(
                hidden_dim, out_dim, attn_dim, ntypes, etypes, None, False, False, 0.0
            )
        )
...
class ieHGCNConv(nn.Module):
    ...
    def __init__(
        ...
        self.linear_q = nn.ModuleDict(
            {ntype: nn.Linear(out_size, attn_size) for ntype in ntypes}
        )
        ...
        with hg.local_scope():
            ...
            for srctype, etype, dsttype in hg.canonical_etypes:
                ...
                dstdata = self.mods[etype](
                    rel_graph, (src_inputs[srctype], dst_inputs[dsttype])
                )
                ...
                attention[dsttype] = torch.cat(
                    (attention[dsttype], edge_attention.unsqueeze(0))
                )
