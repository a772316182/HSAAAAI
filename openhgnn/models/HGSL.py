...
class HGSL(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        ...
        if args.dataset_name == "acm4GTN":
            ...
            raise NotImplemented(
                "HGSL on dataset {} has not been implemented".format(args.dataset_name)
            )
        ...
        return cls(
            ...
            device=device,
            metapaths=metapaths,
            mp_emb_dim=mp_emb_dim,
            hidden_dim=args.hidden_dim,
            num_heads=args.num_heads,
            fs_eps=args.fs_eps,
            fp_eps=args.fp_eps,
            mp_eps=args.mp_eps,
            gnn_emd_dim=args.gnn_emd_dim,
    ...
    def __init__(
        ...
        self.encoder = nnmd(
            dict(
                zip(
                    self.node_types,
                    [
                        nn.Linear(feat_dims[node_type], hidden_dim)
        ...
        for canonical_etype in undirected_relations:
            ...
            self.fgg_direct[undirected_relation] = GraphGenerator(
                hidden_dim, num_heads, fs_eps, self.device
            )
            self.fgg_left[undirected_relation] = GraphGenerator(
                feat_dims[canonical_etype[0]], num_heads, fp_eps, self.device
            )
            self.fgg_right[undirected_relation] = GraphGenerator(
                feat_dims[canonical_etype[2]], num_heads, fp_eps, self.device
            )
            self.fg_agg[undirected_relation] = GraphChannelAttLayer(3)

            # Semantic Graph Generator
            self.sgg_gen[undirected_relation] = nnmd(
                dict(
                    zip(
                        metapaths,
                        [
                            GraphGenerator(mp_emb_dim, num_heads, mp_eps, self.device)
                            for _ in metapaths
                        ],
                    )
                )
        ...
        if len(set(feat_dims.values())) == 1:
            self.GCN = GCN(
                list(self.feat_dims.values())[0], gnn_emd_dim, num_class, gnn_dropout
            )
        else:
            raise Exception(
                "Downstream model GCN can only accept features for "
    ...
    def forward(self, hg, h_features):
        ...
        for ntype in self.node_types:
            if "h" in hg.nodes[ntype].data.keys():
                mapped_feats[ntype] = self.non_linear(
                    self.encoder[ntype](hg.nodes[ntype].data["h"].clone())
                )
            else:
                mapped_feats[ntype] = self.non_linear(
                    self.encoder[ntype](h_features[ntype].clone())
                )
        ...
        for canonical_etype in self.ud_rels:
            undirected_relation = canonical_etype[1]
            ori_g = F.normalize(
                hg.adj(etype=canonical_etype).to_dense().to(self.device), dim=1, p=2
            )

            # Feature Graph Generation
            fg_direct = self.fgg_direct[undirected_relation](
                mapped_feats[canonical_etype[0]], mapped_feats[canonical_etype[2]]
            )
            if (
                "h" in hg.nodes[canonical_etype[0]].data.keys()
                and "h" in hg.nodes[canonical_etype[2]].data.keys()
            ):
                fmat_l, fmat_r = (
                    hg.nodes[canonical_etype[0]].data["h"],
                    hg.nodes[canonical_etype[2]].data["h"],
                )
            else:
                fmat_l, fmat_r = (
                    h_features[canonical_etype[0]],
                    h_features[canonical_etype[2]],
                )
            sim_l, sim_r = self.fgg_left[undirected_relation](
                fmat_l, fmat_l
            ), self.fgg_right[undirected_relation](fmat_r, fmat_r)
            ...
            sem_g_list = [
                self.sgg_gen[undirected_relation][mp](
                    hg.nodes[canonical_etype[0]].data[mp],
                    hg.nodes[canonical_etype[2]].data[mp],
                )
            ...
            # Overall Graph
            new_adjs[canonical_etype] = self.overall_g_agg[undirected_relation](
                [feat_g, sem_g, ori_g]
            )
        ...
        result = {
            self.category: x[
                node_indexes[self.category][0] : node_indexes[self.category][1], :
            ]
        }
        return result
