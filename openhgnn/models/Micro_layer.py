...
class MicroConv(nn.Module):
    ...
    def forward(
        ...
        e_dst = (feat_dst * src_nodes_attention_weight[:, : self._out_feats]).sum(
            dim=-1, keepdim=True
        )
        # Tensor, (N_src, n_heads, 1),   (N_src, n_heads, hidden_dim) * (n_heads, hidden_dim)
        e_src = (feat_src * src_nodes_attention_weight[:, self._out_feats :]).sum(
            dim=-1, keepdim=True
        )
        ...
        dst_features = graph.dstdata.pop("ft").reshape(
            -1, self._num_heads * self._out_feats
        )
    ...
    def forward(
        ...
        for ntype in input_dst:
            if graph.number_of_dst_nodes(ntype) != 0:
                ...
                central_node_feature = torch.matmul(
                    central_node_feature, central_node_transformation_weight[ntype]
                ...
                for relation_tuple in relation_features:
                    ...
                    if dtype == ntype:
                        ...
                        types_features.append(
                            torch.matmul(
                                relation_features[relation_tuple],
                ...
                if types_features.shape[0] == 1:
                    ...
                    # Tensor, (relations_num, N_ntype, n_heads, hidden_dim)
                    types_features = types_features.view(
                        types_features.shape[0], -1, self._num_heads, self._out_feats
                    )
                    # (relations_num, N_ntype, n_heads, hidden_dim)
                    stacked_central_features = torch.stack(
                        [central_node_feature for _ in range(types_features.shape[0])],
                        dim=0,
                    )
                    # (relations_num, N_ntype, n_heads, 2 * hidden_dim)
                    concat_features = torch.cat(
                        (stacked_central_features, types_features), dim=-1
                    )
                    # (relations_num, N_ntype, n_heads, 1) -> (n_heads, 2 * hidden_dim) * (relations_num, N_ntype, n_heads, 2 * hidden_dim)
                    attention_scores = (
                        edge_types_attention_weight * concat_features
                    ...
                    output_feature = output_feature.reshape(
                        -1, self._num_heads * self._out_feats
                    )
