...
class GIE(BaseModel):
    ...
    def __init__(self, args, hg):
        ...
        self.scale = th.Tensor([1.0 / np.sqrt(self.dim)]).double().to(self.device)
        ...
        self.c = nn.Parameter(
            th.ones((self.rel_num, 1), dtype=th.double), requires_grad=True
        )
        self.v = nn.Parameter(
            th.ones((self.rel_num, 1), dtype=th.double), requires_grad=True
        )
    ...
    def forward(self, h, r, t):
        ...
        res_H = self.logmap0(self.givens_rotations(self.r_diagH(r), lh_H), v).view(
            (-1, 1, self.dim)
        )
        ...
        res_S = self.logmap0(self.givens_rotations(self.r_diagS(r), lh_S), u).view(
            (-1, 1, self.dim)
        )
        ...
        hr_emb = self.project(
            self.mobius_add(self.expmap0(th.sum(att_W * cands, dim=1), c), r_exp0c, c),
            c,
        )
    ...
    def givens_rotations(self, r, x):
        ...
        x_rot = givens[:, :, 0:1] * x + givens[:, :, 1:] * th.cat(
            (-x[:, :, 1:], x[:, :, 0:1]), dim=-1
        )
