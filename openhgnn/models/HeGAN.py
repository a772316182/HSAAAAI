...
class Generator(nn.Module):
    ...
    def __init__(self, emb_size, hg):
        ...
        for nodes_type, nodes_emb in hg.ndata["h"].items():
            self.nodes_embedding[nodes_type] = nn.Parameter(
                nodes_emb, requires_grad=True
            )
            ...
            OrderedDict(
                [
                    (
                        "w_1",
                        nn.Linear(
                            in_features=self.node_emb_dim,
                            out_features=self.node_emb_dim,
                            bias=True,
                        ),
                    ),
                    ("a_1", nn.LeakyReLU()),
            ...
            for et in gen_hg.canonical_etypes:
                gen_hg.apply_edges(
                    lambda edges: {
                        "s": edges.src["dh"]
                        .unsqueeze(1)
                        .matmul(edges.data["de"])
                        .squeeze()
                    },
                    etype=et,
                )
                gen_hg.apply_edges(
                    lambda edges: {"score": edges.data["s"].multiply(edges.data["g"])},
                    etype=et,
                )
    ...
    def generate_neighbor_emb(self, hg, noise_emb):
        for et in hg.canonical_etypes:
            hg.apply_edges(
                lambda edges: {
                    "g": edges.src["h"].unsqueeze(1).matmul(edges.data["e"]).squeeze()
                },
                etype=et,
            )
            hg.apply_edges(
                lambda edges: {"g": edges.data["g"] + noise_emb[et]}, etype=et
            )
        ...
        with hg.local_scope():
            for et in hg.canonical_etypes:
                ...
                if len(hg.edata["f"]) == 0:
                    hg.apply_edges(
                        lambda edges: {
                            "score": edges.data["s"].multiply(edges.dst["h"])
                        },
                        etype=et,
                    )
                else:
                    hg.apply_edges(
                        lambda edges: {
                            "score": edges.data["s"].multiply(edges.data["f"])
                        },
                        etype=et,
                    )
