...
class HetGNN(BaseModel):
    ...
    def get_embedding(self):
        ...
        x = self.model(
            self.model.preprocess(self.hg, self.args).to(self.args.device),
            input_features,
        )
        return x
    ...
    def link_preddiction(self):
        ...
        self.model.lp_evaluator(
            x[self.category].to("cpu").detach(), self.train_batch, self.test_batch
        )

    def node_classification(self):
        x = self.get_embedding()
        self.model.nc_evaluator(
            x[self.category].to("cpu").detach(),
            self.labels,
            self.train_idx,
            self.test_idx,
        )
    ...
    def forward(self, hg, h_dict):
        with hg.local_scope():
            ...
            for n in self.ntypes:
                ...
                concat_h = th.hstack(concat_h).view(
                    batch_size * (len(self.ntypes) + 1), self.dim * 2
                )
                atten_w = self.activation(self.atten_w[n](concat_h)).view(
                    batch_size, len(self.ntypes) + 1
                )

                atten_w = self.softmax(atten_w).view(batch_size, 1, 4)

                # weighted combination
                concat_emd = th.hstack(concat_emd).view(
                    batch_size, len(self.ntypes) + 1, self.dim
                )

                weight_agg_batch = th.bmm(atten_w, concat_emd).view(
                    batch_size, self.dim
                )
...
class het_content_encoder(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.content_rnn = nn.LSTM(
            dim, int(dim / 2), 1, batch_first=True, bidirectional=True
        )
        ...
        with hg.local_scope():
            ...
            if isinstance(inputs, tuple) or hg.is_block:
                if isinstance(inputs, tuple):
                    ...
                    src_inputs = inputs
                    dst_inputs = {
                        k: v[: hg.number_of_dst_nodes(k)] for k, v in inputs.items()
                    }
                ...
                for stype, etype, dtype in hg.canonical_etypes:
                    ...
                    dstdata = self.neigh_rnn[stype](
                        rel_graph, (src_inputs[stype], dst_inputs[dtype])
                    )
