    ...
    **Authors:** Bo Yan, Cheng Yang, Chuan Shi, Jiawei Liu, Xiaochen Wang
    ...
    def build_model_from_args(cls, args):
        return AEHCL(
            ...
            num_of_attention_heads=args.num_of_attention_heads,
            batch_size=args.batch_size,
            all_len=args.all_len,
            weight_inter=args.weight_inter,
            weight_intra_multi=args.weight_intra_multi,
            weight_intra_pair=args.weight_intra_pair,
            t=args.t,
        )
    ...
    def __init__(
        ...
        self.BCE_loss = nn.BCEWithLogitsLoss(
            reduction="none", pos_weight=torch.tensor([1.0], dtype=torch.float32)
        )
    ...
    def forward(
        self, event, pos_event, neg_event, neg_context, neg_entity, event_mask, type_num
    ):
        ...
        for key in event.keys():
            if key != self.center_type:
                ...
                event_context_z[key] = self.fc[key](
                    event_context[key]
                )  # (batch,len[key],out_dim)
        ...
        for key in event_context_z.keys():
            if pos_context is None:
                ...
                pos_context = torch.cat(
                    (pos_context, event_context_z[key]), 1
                )  # (batch,all_len-1,out_dim)
                type_num_ = torch.cat(
                    (type_num_, type_num[key]), 1
                )  # (batch,all_len-1)
        ...
        for key in event_context_z.keys():
            pos_entity_z = torch.cat(
                (pos_entity_z, event_context_z[key]), 1
            )  # (batch,all_len,out_dim)
            pos_entity_mask = torch.cat(
                (pos_entity_mask, event_mask[key]), -1
            )  # (batch,all_len)
        pos_norm = torch.norm(pos_entity_z, dim=-1, keepdim=True)
        pos_entity_z = pos_entity_z / pos_norm  # (batch,all_len,out_dim)
        pos_matrix = torch.matmul(
            pos_entity_z, torch.transpose(pos_entity_z, 1, 2)
        )  # (batch,all_len,all_len)
        pos_matrix = torch.exp(pos_matrix / self.t)  # (batch,all_len,all_len)
        mask = torch.ones_like(pos_matrix) - torch.eye(pos_matrix.shape[1]).to(
            pos_matrix.device
        )
        ...
        pos_entity_mask_ = pos_entity_mask_.expand(
            pos_matrix_.shape
        )  # (batch,all_len,all_len)
        ...
        for key in neg_entity.keys():
            if neg_entity_z is None:
                ...
                neg_entity_z = torch.cat(
                    (neg_entity_z, self.fc[key](neg_entity[key])), 1
                )  # (batch,all_len,neg_num,out_dim)
        neg_norm = torch.norm(
            neg_entity_z, dim=-1, keepdim=True
        )  # (batch,all_len,neg_num,1)
        neg_entity_z = neg_entity_z / neg_norm  # (batch,all_len,neg_num,out_dim)
        neg_matrix = torch.matmul(
            torch.unsqueeze(pos_entity_z, 2), torch.transpose(neg_entity_z, 2, 3)
        )  # (batch,all_len,1,neg_num)
        ...
        lbl = torch.unsqueeze(
            torch.cat(
                (torch.ones(pos_inter.shape[0]), torch.zeros(neg_inter.shape[0]))
            ),
            1,
        )  # (2*batch,1)
        ...
        intra_pair = -torch.log(
            (pos_ + 1e-10) / (pos_ + neg_ + 1e-10)
        )  # (batch,all_len)
        ...
        loss = (
            self.weight_inter * inter_loss
            + self.weight_intra_multi * intra_multi_loss
            + self.weight_intra_pair * intra_pair_loss
        )
        ...
        if self.training:
            ...
            intra_pair_score = torch.FloatTensor(intra_pair_score).to(
                pos_matrix.device
            )  #
            intra_pair_score = self.sigmoid(intra_pair_score)

            score = (
                -self.weight_inter * inter_score
                - self.weight_intra_multi * intra_multi_score
                - self.weight_intra_pair * intra_pair_score
            )  # (batch)
...
class Attention(nn.Module):
    ...
    def forward(self, event):
        ...
        for key in event.keys():
            if key != self.center_type:
                ...
                if context_k is None:
                    ...
                    context_z = torch.cat(
                        (context_z, event[key]), 1
                    )  # (batch,all_len-1,out_dim)
                    context_k = torch.cat(
                        (context_k, context_k_), 1
                    )  # (batch,all_len-1,out_dim)

        context_coef = context_k.matmul(
            torch.transpose(center, 1, 2)
        )  # (batch,all_len-1,1)
        ...
        cur_event = torch.cat(
            (context_aggre, torch.squeeze(center)), 1
        )  # (batch,out_dim*2)
...
class SelfAttention(nn.Module):
    def __init__(
        self, center_type, context_type, out_dim, num_of_attention_heads, type_n
    ):
        super().__init__()
    ...
    def forward(self, hidden_states, type_num):  # (batch,all_len-1,out_dim)
        ...
        query_layer = self.transpose_for_scores(
            mixed_query_layer
        )  # (batch,num_of_head,all_len-1,head_size)
        ...
        )  # (batch,num_of_head,all_len-1,all_len-1)
        ...
        context_layer = context_layer.permute(
            0, 2, 1, 3
        ).contiguous()  # (batch,all_len-1,num_of_head,head_size)

        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(
            *new_context_layer_shape
        )  # (batch,all_len-1,all_head_size)
