...
class Model(nn.Module):
    ...
    def _generate_model_layer(self):
        ...
        self._word_embed = nn.Parameter(
            Variable(
                torch.Tensor(self.args.vocab, self.embed_size_word),
                requires_grad=True,
            )
        )
    ...
    def _cnn(self, features_num):
        ...
        conv = nn.Conv2d(
            1, 12, kernel_size=(2, features_num), stride=1, padding="valid"
        )
    ...
    def forward(self, x):
        ...
        inputs_query_raw = torch.nn.functional.embedding(
            query_terms.to(torch.int64), self._word_embed
        )
        input_num = (
            torch.sum(torch.sign(query_terms))
            if torch.sum(torch.sign(query_terms)) > 1
            else torch.tensor(1)
        )
        # get query word to vec sum 64 * batchsize
        self.query_w2v_sum = torch.mean(
            inputs_query_raw[: int(input_num.item())], 0
        ).T  # 64*512
        ...
        for raw_word_embed in raw_word_embedding_list:
            item_terms, item_topcate, item_leafcate, time_delta = torch.split(
                raw_word_embed, [self.user_item_term_length, 1, 1, 1], 0
            )

            step_embedding = torch.nn.functional.embedding(
                item_terms.to(torch.int64), self._word_embed
            )

            input_num = (
                torch.sum(torch.sign(item_terms))
                if torch.sum(torch.sign(item_terms)) > 1
                else torch.tensor(1)
            )
        ...
        lstm_cells, lstm_hiddens = self.user_word_lstm(
            step_embedding_vec, (self.h_1, self.c_1)
        )
        ...
        self.query_item_query_cnn_output = self.query_item_query_relu(
            self.query_item_query_linear(pool_flat)
        ).T
        ...
        self.query_user_item_cnn_output = self.query_user_item_relu(
            self.query_user_item_linear(pool_flat)
        ).T
        ...
        for raw_user_item_embed in raw_user_query_seq_embedding_list[::-1]:
            (
                ...
                raw_user_item_embed, [self.user_query_term_length, 3, 3, 1], 0
            )
        ...
        self.wide_hidden_layer1 = torch.tanh(  # [64, 81] * [81, 1]
            F.dropout(
                torch.matmul(self.wide_feat_w, wide_feat.float()) + self.wide_feat_b,
                self.keep_prob,
            )
        )
        concat_seq = [
        ...
        qu_term_concat = F.dropout(
            torch.cat(concat_seq, 0), self.keep_prob
        )  # [64*7, 1]
        # concat user query feats: 64 * batch_size
        self.qu_term_hidden_layer1 = torch.tanh(
            F.dropout(
                torch.matmul(
                    self.concat_query_w, qu_term_concat
                )  # 聚合特征信息 [64, 64*7] * [64*7, 1]
                + self.concat_query_b,
                self.keep_prob,
            )
        )
