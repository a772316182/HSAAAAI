    ...
    def build_model_from_args(cls, args, hg):
        ...
        if args.dataset == "imdb4MAGNN":
            ...
            raise NotImplementedError(
                "MAGNN on dataset {} has not been implemented".format(args.dataset)
            )
    ...
    def __init__(
        ...
        for i in range(num_layers - 1):
            self.layers.append(
                MAGNN_layer(
                    in_feats=h_feats,
                    inter_attn_feats=inter_attn_feats,
                    out_feats=h_feats,
                    num_heads=num_heads,
                    metapath_list=metapath_list,
                    ntypes=self.ntypes,
                    edge_type_list=edge_type_list,
                    dst_ntypes=self.dst_ntypes,
...
class MAGNN_layer(nn.Module):
    def __init__(
        ...
        for metapath in self.metapath_list:
            self.intra_attn_layers[metapath] = MAGNN_attn_intra(
                in_feats=in_feats_dst_meta, out_feats=in_feats, num_heads=num_heads
            )
        ...
        for ntype in dst_ntypes:
            self.inter_linear[ntype] = nn.Linear(
                in_features=in_feats * num_heads,
                out_features=inter_attn_feats,
                bias=True,
            )
            self.inter_attn_vec[ntype] = nn.Linear(
                in_features=inter_attn_feats, out_features=1, bias=False
            )
        ...
        if encoder_type == "RotateE":
            ...
            r_vec_ = nn.Parameter(
                th.empty(size=(len(edge_type_list) // 2, in_feats * num_heads // 2, 2))
            )
        ...
        # This part is a little different from the original author's codes.
            ...
            self.encoder_linear = nn.Linear(
                in_features=in_feats * num_heads, out_features=in_feats * num_heads
            )

        # output layer
        if last_layer:
            self._output_projection = nn.Linear(
                in_features=num_heads * in_feats, out_features=out_feats
            )
        else:
            self._output_projection = nn.Linear(
                in_features=num_heads * in_feats, out_features=num_heads * out_feats
            )
    ...
    def forward(self, feat_dict, metapath_idx_dict):
        ...
        for _metapath in self.metapath_list:
            feat_intra[_metapath] = self.intra_metapath_trans(
                feat_dict, metapath=_metapath, metapath_idx_dict=metapath_idx_dict
            )

        # Inter-metapath latent transformation
        feat_inter = self.inter_metapath_trans(
            feat_dict=feat_dict, feat_intra=feat_intra, metapath_list=self.metapath_list
        )
    ...
    def intra_metapath_trans(self, feat_dict, metapath, metapath_idx_dict):
        ...
        feat_intra = self.intra_attn_layers[metapath](
            [intra_metapath_feat, feat_dict[metapath.split("-")[0]]],
            metapath,
            metapath_idx,
        )
        return feat_intra
    ...
    def inter_metapath_trans(self, feat_dict, feat_intra, metapath_list):
        ...
        for metapath in metapath_list:
            ...
            meta_feat = th.tanh(self.inter_linear[_metapath[0]](meta_feat)).mean(
                dim=0
            )  # s_pi
        ...
        for ntype in self.ntypes:
            if ntype in self.dst_ntypes:
                ...
                metapaths = np.array(metapath_list)[
                    [meta.split("-")[0] == ntype for meta in metapath_list]
                ]
                ...
                feat_inter[ntype] = th.stack(
                    [meta_b[i] * meta_feat[i] for i in range(len(meta_b))], dim=0
                ).sum(dim=0)
            else:
    ...
    def encoder(self, feat_dict, metapath, metapath_idx):
        ...
        feat = th.zeros(
            (len(_metapath), metapath_idx.shape[0], feat_dict[_metapath[0]].shape[1]),
            device=device,
        )
        ...
        if self.encoder_type == "RotateE":
            ...
            for i in range(1, len(_metapath), 1):
                ...
                temp_r_vec[i] = self.complex_hada(
                    temp_r_vec[i - 1], self.r_vec_dict[edge_type]
                )
            ...
            raise ValueError(
                "The encoder type {} has not been implemented yet.".format(
                    self.encoder_type
                )
            )
...
class MAGNN_attn_intra(nn.Module):
    ...
    def forward(self, feat, metapath, metapath_idx):
        ...
        h_meta = self.feat_drop(feat[0]).view(
            -1, self._num_heads, self._out_feats
        )  # feature matrix of metapath instances

        # metapath(right) part of attention
        er = (h_meta * self.attn_r).sum(dim=-1).unsqueeze(-1)

        graph_data = {
            ("meta_inst", "meta2{}".format(_metapath[0]), _metapath[0]): (
                th.arange(0, metapath_idx.shape[0]),
                th.tensor(metapath_idx[:, 0]),
            )
        }
        num_nodes_dict = {
            "meta_inst": metapath_idx.shape[0],
            _metapath[0]: feat[1].shape[0],
        }
        ...
        g_meta.apply_edges(
            func=fn.copy_u("er", "e"), etype="meta2{}".format(_metapath[0])
        )
        ...
        g_meta.update_all(
            message_func=fn.u_mul_e("feat_src", "a", "m"),
    ...
    if os.path.exists(file_addr) and test is False:  # TODO
        ...
        for metapath in metapath_list:
            ...
            for i in range(1, len(_metapath) - 1):
                ...
                res[metapath] = pd.merge(
                    res[metapath], feat_j, left_on=col_i, right_on=col_j, how="inner"
                )
                if col_i != col_j:
...
def mini_mp_instance_sampler(seed_nodes, mp_instances, num_samples):
    ...
    for ntype in seed_nodes.keys():
        target_mp_types = np.array(metapath_list)[
            [meta.split("-")[0] == ntype for meta in metapath_list]
        ]
        for (
            metapath
        ) in target_mp_types:  # the metapath instances of the certain metapath
            for node in seed_nodes[ntype]:
                ...
                if metapath not in mini_mp_inst.keys():
                    ...
                    mini_mp_inst[metapath] = np.concatenate(
                        (mini_mp_inst[metapath], _mp_inst[mp_choice]), axis=0
                    )
