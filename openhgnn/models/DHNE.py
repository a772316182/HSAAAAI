...
class DHNE(BaseModel):
    ...
    **Authors:** Ke Tu, Peng Cui, Xiao Wang, Fei Wang, Wenwu Zhu
    ...
    def __init__(self, nums_type, dim_features, embedding_sizes, hidden_size, device):
        ...
        self.encodeds = [
            nn.Linear(
                sum([self.nums_type[j] for j in range(3) if j != i]),
        ...
        self.hidden_layer = nn.Linear(sum(self.embedding_sizes), self.hidden_size)
    ...
    def forward(self, input_ids):
        ...
        for i in range(3):
            encoded = torch.tanh(
                self.encodeds[i].to(self.device)(input_ids[i].to(self.device))
            )
