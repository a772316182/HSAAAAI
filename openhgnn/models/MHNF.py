...
class MHNF(BaseModel):
    ...
    def forward(self, hg, h=None):
        with hg.local_scope():
            ...
            self.A, h, self.category_idx = transform_relation_graph_list(
    ...
    def forward(self, A, h):
        ...
        for i in range(self.num_channels):
            ...
            channel_attention_list.append(
                th.bmm(
                    th.stack(layer_level_feature_list, dim=-1),
                    layer_level_beta.unsqueeze(-1),
                ).squeeze(-1)
            )
        ...
        for i in range(self.num_channels):
            ...
            if i == 0:
                ...
                channel_level_alpha = th.cat(
                    (
                        channel_level_alpha,
                        self.channel_attention(channel_level_feature),
                    ),
                    dim=-1,
                )
        channel_level_beta = th.softmax(channel_level_alpha, dim=-1)
        channel_attention = th.bmm(
            th.stack(channel_attention_list, dim=-1), channel_level_beta.unsqueeze(-1)
        ).squeeze(-1)
        ...
        for i in range(self.num_channels):
            ...
            for j in range(len(layer_list)):
                ...
                edge_weight = th.cat(
                    (
                        edge_weight,
                        th.full((layer.number_of_nodes(),), 1, device=layer.device),
                    )
                )
...
class HMAELayer(nn.Module):
    ...
    def softmax_norm(self, H):
        ...
        for i in range(len(H)):
            g = H[i]
            g.edata["w_sum"] = self.norm(
                g, th.exp(g.edata["w_sum"])
            )  # normalize the hybrid relationship matrix
            norm_H.append(g)
        return norm_H
...
class GTConv(nn.Module):
    ...
    def forward(self, A):
        A = list(filter(lambda x: x.num_edges() > 0, A))
