...
class GTN(BaseModel):
    ...
    def __init__(
        ...
        self.gcn = GraphConv(
            in_feats=self.in_dim, out_feats=hidden_dim, norm="none", activation=F.relu
        )
    ...
    def forward(self, hg, h):
        with hg.local_scope():
            ...
            for i in range(self.num_channels):
                ...
                edge_weight = th.cat(
                    (edge_weight, th.full((g.number_of_nodes(),), 1, device=g.device))
                )
...
class GTLayer(nn.Module):
    ...
    def forward(self, A, H_=None):
        if self.first:
            ...
            W = [
                (F.softmax(self.conv1.weight, dim=1)).detach(),
                (F.softmax(self.conv2.weight, dim=1)).detach(),
            ]
        else:
            result_A = H_
