...
class HGNN_AC(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        return cls(
            in_dim=hg.nodes[hg.ntypes[0]].data["h"].shape[1],
    ...
    def __init__(self, in_dim, hidden_dim, dropout, activation, num_heads, cuda):
        ...
        self.attentions = [
            AttentionLayer(in_dim, hidden_dim, dropout, activation, cuda)
    ...
    def forward(self, bias, emb_dest, emb_src, feature_src):
        ...
        x = torch.cat(
            [
                att(adj, emb_dest, emb_src, feature_src).unsqueeze(0)
    ...
    def __init__(self, in_dim, hidden_dim, dropout, activation, cuda=False):
        ...
        self.W = nn.Parameter(
            nn.init.xavier_normal_(
                torch.Tensor(in_dim, hidden_dim).type(
        ...
        self.W2 = nn.Parameter(
            nn.init.xavier_normal_(
                torch.Tensor(hidden_dim, hidden_dim).type(
                    torch.cuda.FloatTensor if cuda else torch.FloatTensor
                ),
                gain=np.sqrt(2.0),
            ),
