...
class TransD(BaseModel):
    ...
    def _transfer(self, n, n_emb_p, r_emb_p):
        if n.shape[0] != r_emb_p.shape[0]:
            ...
            n = F.normalize(
                self._resize(n, -1, r_emb_p.size()[-1])
                + th.sum(n * n_emb_p, -1, True) * r_emb_p,
                p=2,
                dim=-1,
            )
            ...
            return F.normalize(
                self._resize(n, -1, r_emb_p.shape[-1])
                + th.sum(n * n_emb_p, -1, True) * r_emb_p,
                p=2,
                dim=-1,
            )
    ...
    def forward(self, h, r, t):
        if self.training:
            ...
            self.n_emb_p.weight.data = F.normalize(
                self.n_emb_p.weight.data, p=2.0, dim=-1
            )
            self.r_emb_p.weight.data = F.normalize(
                self.r_emb_p.weight.data, p=2.0, dim=-1
            )
