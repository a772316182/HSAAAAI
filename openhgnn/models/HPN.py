...
class HPN(BaseModel):
    ...
    def __init__(
        ...
        self.layers.append(
            HPNLayer(meta_paths, in_size, dropout, k_layer, alpha, edge_drop)
        )
...
class HPNLayer(nn.Module):
    ...
    def __init__(self, meta_paths_dict, in_size, dropout, k_layer, alpha, edge_drop):
        ...
        self.hidden = nn.Sequential(
            nn.Linear(in_features=in_size, out_features=in_size, bias=True), nn.ReLU()
        )
        self.meta_paths_dict = meta_paths_dict

        semantic_attention = SemanticAttention(in_size=in_size)
        mods = nn.ModuleDict(
            {mp: APPNPConv(k_layer, alpha, edge_drop) for mp in meta_paths_dict}
        )
