...
class SHGPTrainer(BaseFlow):
    ...
    def __init__(self, args):
        ...
        self.layer_shape = []
        self.input_layer_shape = dict(
            [(k, self.ft_dict[k].shape[1]) for k in self.ft_dict.keys()]
        )
        self.hidden_layer_shape = [
            dict.fromkeys(self.ft_dict.keys(), l_hid) for l_hid in args.hidden_dim
        ]
        ...
        self.net_schema = dict(
            [(k, list(self.adj_dict[k].keys())) for k in self.adj_dict.keys()]
        )
    ...
    def train(self):
        ...
        if not self.args.load_from_pretrained:
            ...
            optimizer = torch.optim.Adam(
                self.model.parameters(), lr=self.args.lr, weight_decay=self.args.l2_coef
            )
            for epoch in range(self.args.epochs):
                ...
                if epoch == 0:
                    init_pseudo_label = init_lpa(
                        self.adj_dict, self.ft_dict, self.target_type, self.num_cluster
                    )
    ...
    def evaluate(
        self, embeds, idx_train, idx_val, idx_test, labels, num_class, isTest=True
    ):
        print(
            "----------------------------start evaluating----------------------------------"
        )
...
def lpa(init_label_dict, adj_dict, target_type, num_cluster, max_iter=1000):
    ...
    for i in range(max_iter):
        for k in net_schema:
            ...
            new_k_label = torch.cat(
                [nb_label.unsqueeze(1) for nb_label in k_nbs_label_list], 1
            )
...
def att_lpa(
    adj_dict, init_pseudo_label, attention_dict, target_type, num_cluster, max_iter=1000
):
    ...
    for _ in range(max_iter):
        for m in range(len(attention_dict)):
            for k in net_schema:
                ...
                pseudo_label_dict[k] = (
                    ...
                    .mul(attention_dict[m][k].unsqueeze(-1))
                    .sum(1)
                )
        ...
        for k in net_schema:
            ...
            if k == target_type:
                target_label_list.append(new_k_label)
            pseudo_label_dict[k] = F.one_hot(new_k_label, num_cluster).type(
                dtype=torch.float32
            )
            current_label_dict[k] = pseudo_label_dict[k]
        if len(target_label_list) > 1:
