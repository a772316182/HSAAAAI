...
class DSSL_trainer(BaseFlow):
    ...
    def __init__(self, args=None):
        ...
        if args.rand_split == True or args.dataset in [
            "snap-patents",
            "ogbn-proteins",
            "wiki",
            "Cora",
        ...
        self.split_idx_lst = [
            {"train": self.train_idx, "valid": self.val_idx, "test": self.test_idx}
        ]
        self.pred_idx = getattr(self.task.dataset, "pred_idx", None)

        self.labels = self.task.get_labels().to(self.device)
        self.num_nodes_dict = {
            ntype: self.hg.num_nodes(ntype) for ntype in self.hg.ntypes
        }
        self.to_homo_flag = getattr(self.model, "to_homo_flag", False)

        self.sampled_neighborhoods = self.sample_neighborhood(
            self.task.dataset, self.device, args
        )
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):  # args.epochs = 200
            ...
            for batch in range(0, self.task.dataset.g.num_nodes(), batch_size):
                ...
                ]  # 从图中随机取batch_size个结点的id
                batch_idx = batch_idx.to(self.device)
                batch_neighbor_index = self.sampled_neighborhoods[
                    batch_idx
                ]  # 每个随机取的原结点取出有neighbor_max的邻居结点
                batch_embedding = online_embedding[batch_idx].to(
                    self.device
                )  # 随机选的batch_size个结点的online_embedding
                batch_embedding = F.normalize(
                    batch_embedding, dim=-1, p=2
                )  # 归一化 尺寸为   batch_size*dim=【1024，64】

                # target_embedding 中拿出随机原结点的邻居节点
                batch_neighbor_embedding = [
                    target_embedding[i, :].unsqueeze(0) for i in batch_neighbor_index
                ]
                batch_neighbor_embedding = torch.cat(
                    batch_neighbor_embedding, dim=0
                ).to(self.device)
                batch_neighbor_embedding = F.normalize(
                    batch_neighbor_embedding, dim=-1, p=2
                )
                # batch_size*neighbor_max*dim=【1024，5，64】

                main_loss, context_loss, entropy_loss, k_node = self.model(
                    batch_embedding, batch_neighbor_embedding
                )
                tmp = (
                    F.one_hot(
                        torch.argmax(k_node, dim=1), num_classes=self.args.cluster_num
                    )
                ...
                if self.args.neg_alpha:  # args.neg_alpha = 0
                    ...
                    batch_neighbor_embedding = [
                        ...
                        for i in batch_neg_neighbor_index
                    ]
                    batch_neighbor_embedding = torch.cat(
                        batch_neighbor_embedding, dim=0
                    ).to(device)
                    ...
                    main_neg_loss, tmp, tmp, tmp = model(
                        batch_embedding, batch_neighbor_embedding
                    )
                    loss = (
                        main_loss
                        + args.gamma * (context_loss + entropy_loss)
                        + main_neg_loss
                    )
                ...
                print(
                    "batch : {}, main_loss: {}, context_loss: {}, entropy_loss: {}".format(
                        batch, main_loss, context_loss, entropy_loss
                    )
                )
                loss.backward()
                optimizer.step()
            ...
            if epoch % 1 == 0:
                ...
                for batch in range(0, self.task.dataset.g.num_nodes(), batch_size):
                    ...
                    tmp = (
                        F.one_hot(
                            torch.argmax(k_node, dim=1),
                            num_classes=self.args.cluster_num,
                        )
                        .type(torch.FloatTensor)
                        .to(self.device)
                    )
            ...
            if epoch % 10 == 0:
                ...
                print(timetmp)
                train_acc, dev_acc, test_acc = self.evaluate(
                    self.model, self.task, split_idx
                )
                print(
    ...
    def sample_neighborhood(self, dataset, device, args):
        ...
        for i in range(0, dataset.g.num_nodes()):
            ...
            if num_neighbor < neighbor_max:
                ...
                adjacent_list[i] = adjacent_list[i].repeat(1, more)[
                    0, :neighbor_max
                ]  # 邻居数量不够使用重复填充
            if num_neighbor > neighbor_max:
                perm = torch.randperm(num_neighbor)
                idx = perm[:neighbor_max]
                adjacent_list[i] = adjacent_list[i][
                    idx
                ]  # 邻居数量超过使用在原num_neighbor里随机取neighbor_max个
    ...
    def evaluate(self, model, task, split_idx):
        ...
        for i in range(10):
            ...
            optimizer_LR = torch.optim.AdamW(
                classifier.parameters(), lr=0.01, weight_decay=0.01
            )

            for epoch in range(100):  # epoch（LR） = 100
                classifier.train()
                logits, loss = classifier(
                    embedding[train_idx, :], train_label.squeeze()
                )
            ...
            train_acc = (
                (
                    torch.sum(train_preds == train_label.squeeze()).float()
                    / train_label.squeeze().shape[0]
                )
                .detach()
                .cpu()
                .numpy()
            ...
            dev_acc = (
                (
                    torch.sum(dev_preds == valid_label.squeeze()).float()
                    / valid_label.squeeze().shape[0]
                )
                .detach()
                .cpu()
                .numpy()
            ...
            test_acc = (
                (
                    torch.sum(test_preds == test_label.squeeze()).float()
                    / test_label.squeeze().shape[0]
                )
                .detach()
                .cpu()
                .numpy()
