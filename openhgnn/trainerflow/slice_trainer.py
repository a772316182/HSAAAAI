...
class SLiCETrainer(BaseFlow):
    def __init__(self, args):
        ...
        self.pretrain_save_path = os.path.join(
            self.pretrain_path, "best_pretrain_model.pt"
        )
        self.finetune_path = os.path.join(self.out_dir, "finetune/")
        self.finetune_save_path = os.path.join(
            self.finetune_path, "best_finetune_model.pt"
        )
        self.g = self.task.dataset.g
        self.g = dgl.to_homogeneous(
            self.g, edata=["train_mask", "valid_mask", "test_mask", "label"]
        )
        ...
        # optimizer
        self.optimizer = dict()
        self.optimizer["pretrain"] = optim.Adam(
            self.model["pretrain"].parameters(), lr=args.lr
        )
        self.optimizer["finetune"] = optim.Adam(
            self.model["finetune"].parameters(), lr=args.ft_lr
        )
    ...
    def preprocess(self):
        ...
        self.sampler = SLiCESampler(
            self.g,
            self.graphs["train"],
            num_walks_per_node=self.args.n_pred,
            beam_width=self.args.beam_width,
            max_num_edges=self.args.max_length,
            walk_type=self.args.walk_type,
            path_option=self.args.path_option,
        ...
        if os.path.exists(train_file):
            ...
            self.edges["train"], edges_label["train"] = sampler.generate_false_edges2(
                self.edges["train"], train_file
            )
        self.edges["valid"], self.edges_label["valid"] = sampler.shuffle_edge_label(
            self.edges["valid"], self.edges_label["valid"]
        )
        self.edges["test"], self.edges_label["test"] = sampler.shuffle_edge_label(
            self.edges["test"], self.edges_label["test"]
        )
        ...
        for task in ["train", "valid", "test"]:
            ...
            for batch in range(n_batch):
                ...
                batch_file = os.path.join(
                    self.finetune_path, "{}/edge_subgraph_{}.bin".format(task, batch)
                )
                # pair_file=self.finetune_path+'{}/pair_subgraph_{}.pickle'.format(task,batch)
                if not os.path.exists(batch_file):
                    subgraph_list = self.sampler.get_edge_subgraph(
                        self.edges[task][i:end]
                    )
    ...
    def finetune(self):
        ...
        for epoch in range(self.n_epochs["finetune"]):
            ...
            for batch in bar:
                ...
                batch_file = os.path.join(
                    self.finetune_path, "{}/edge_subgraph_{}.bin".format("train", batch)
                )
                ...
                loss = F.binary_cross_entropy(
                    pred_scores,
                    torch.tensor(self.edges_label["train"][i:end], dtype=torch.float)
                    .reshape(-1, 1)
                    .cuda(),
                )
                bar.set_description("Batch {}: Loss:{:.3f}".format(batch, loss))
            ...
            torch.save(
                self.model["finetune"].state_dict(),
                self.finetune_path + "model_" + str(epoch) + "SLiCE.pt",
            )
            avg_loss = avg_loss / n_batch
            print("AvgLoss: {:.3f}".format(avg_loss))
        ...
        with torch.no_grad():
            ...
            for task in ["valid", "test"]:
                ...
                for batch in range(n_batch):
                    ...
                    batch_file = os.path.join(
                        self.finetune_path,
                        "{}/edge_subgraph_{}.bin".format(task, batch),
                    )
                    if os.path.exists(batch_file):
                        subgraph_list, _ = dgl.load_graphs(batch_file)
                    else:
                    ...
                    if not self.is_finetuned:
                        ...
                        score = torch.bmm(source_embed, target_embed).squeeze(
                            1
                        )  # embedding相乘得到相似度分数
            ...
            print(f"{header} : ROC-AUC: {roc_auc}," f" F1: {f1}, AUC: {auc_value}")
