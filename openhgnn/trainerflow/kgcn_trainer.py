...
class KGCNTrainer(BaseFlow):
    ...
    def KGCNCollate(self, index):
        ...
        dataloader = dgl.dataloading.DataLoader(
            self.hg,
            th.LongTensor(inputData[:, 1]).to(device=self.hg.device),
            sampler,
    ...
    def loss_calculation(self):
        ...
        self.l2_loss = (
            th.norm(self.user_emb_matrix) ** 2 / 2
            + th.norm(self.entity_emb_matrix) ** 2 / 2
            + th.norm(self.relation_emb_matrix) ** 2 / 2
        )
        """
    ...
    def renew_weight(self, inputData):
        ...
        weight = th.mm(
            self.relation_emb_matrix[self.hg.edata["relation"].cpu().numpy()],
