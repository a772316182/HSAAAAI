...
class TransXTrainer(BaseFlow):
    ...
    def preprocess(self):
        ...
        if self.args.score_fn == "transr":  # load transe data when training transr
            transe_state_dict = th.load(
                self.stopper.save_path.replace("TransR", "TransE")
            )
    ...
    def _train(self):
        ...
        for epoch in range(self.max_epoch):
            ...
            for iter in iter_range:
                ...
                h_emb, r_emb, t_emb = (
                    th.cat((pos_g[0], neg_g[0]), -1),
                    th.cat((pos_g[1], neg_g[1]), -1),
                    th.cat((pos_g[2], neg_g[2]), -1),
                )
            ...
            if epoch % self.evaluate_interval == 0:
                ...
                self.logger.info(
                    "[Evaluation metric] " + str(val_metric)
                )  # out test result
                early_stop = self.stopper.loss_step(
                    val_metric["valid"]["MR"], self.model
                )  # less is better
                if early_stop:
