...
class lightGCNTrainer(BaseFlow):
    ...
    def __init__(self, args):
        ...
        self.model = (
            ...
            .build_model_from_args(self.args, self.g_dict)
            .to(self.device)
        )
    ...
    def train(self):
        ...
        for epoch in range(self.args.max_epoch):
            ...
            for batch_i, (batch_users, batch_pos, batch_neg) in enumerate(
                self.minibatch(
                    users, posItems, negItems, batch_size=self.args.batch_size
                )
            ):
        ...
        def forward(
            ...
            if self.lambda_reg != 0:
                regularization = (
                    self.lambda_reg
                    * (1 / 2)
                    * parameters.norm(2).pow(2)
                    / float(parameters.shape[0])
                )
            # print(-log_prob)
            #
