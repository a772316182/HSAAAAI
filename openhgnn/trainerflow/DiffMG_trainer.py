    ...
    def preprocess(self):
        if self.args.model == "DiffMG":
            ...
            train_graph = dgl.heterograph(
                {("user", "ui", "item"): train_edge},
                {
                    ntype: self.train_hg.number_of_nodes(ntype)
...
def preprocess_amazon(self):
    ...
    offsets = {
        "i": hg.num_nodes("user"),
        "b": hg.num_nodes("item") + hg.num_nodes("user"),
    }
    ...
    self.train_pos = find_pairs(
        "pos", self.train_hg, self.hg, "ui", self.hg.num_nodes("user")
    )
    self.train_neg = find_pairs(
        "neg", self.train_hg, self.hg, "ui", self.hg.num_nodes("user")
    )
    self.test_pos = find_pairs(
        "pos", self.test_hg, self.hg, "ui", self.hg.num_nodes("user")
    )
    self.test_neg = find_pairs(
        "neg", self.test_hg, self.hg, "ui", self.hg.num_nodes("user")
    )
    self.val_pos = find_pairs(
        "pos", self.val_hg, self.hg, "ui", self.hg.num_nodes("user")
    )
    self.val_neg = find_pairs(
        "neg", self.val_hg, self.hg, "ui", self.hg.num_nodes("user")
    )
    ...
    edge_dict = {
        "i": [ui_edge[0], ui_edge[1]],
        "b": [ib_edge[0], ib_edge[1]],
        "c": [ic_edge[0], ic_edge[1]],
        "v": [iv_edge[0], iv_edge[1]],
    }
    i = 1
    adjs_offset = {}
    for k, v in edge_dict.items():
        adj_offset = np.zeros(
            (node_types.shape[0], node_types.shape[0]), dtype=np.float32
        )
...
def process(self):
    ...
    if "0" in self.adjs_offset:
        adjs_pt.append(
            sparse_mx_to_torch_sparse_tensor(
                normalize_sym(
                    self.adjs_offset["0"]
                    + sp.eye(self.adjs_offset["0"].shape[0], dtype=np.float32)
                )
            )
        )
    for i in range(1, int(max(self.adjs_offset.keys())) + 1):
        adjs_pt.append(
            sparse_mx_to_torch_sparse_tensor(
                normalize_row(
                    self.adjs_offset[str(i)]
                    + sp.eye(self.adjs_offset[str(i)].shape[0], dtype=np.float32)
                )
            )
        )
        adjs_pt.append(
    ...
    adjs_pt.append(
        sparse_mx_to_torch_sparse_tensor(
            sp.eye(self.adjs_offset["1"].shape[0], dtype=np.float32).tocoo()
        )
    )
    ...
    self.serach_adjs_pt.append(
        torch.sparse.FloatTensor(size=self.adjs_offset["1"].shape)
    )
    node_feats = []
    for k in range(num_node_types):
        i = torch.stack(
            (
                torch.arange(self.in_dims[k], dtype=torch.long),
                torch.arange(self.in_dims[k], dtype=torch.long),
            )
        )
        ...
        node_feats.append(
            torch.sparse.FloatTensor(
                i, v, torch.Size([self.in_dims[k], self.in_dims[k]])
            )
        )
    ...
    model_s = self.model(
        self.in_dims, self.args.attn_dim, steps_s, dropout=self.args.dropout
    )
    model_t = self.model(
        self.in_dims, self.args.attn_dim, steps_t, dropout=self.args.dropout
    )
...
def train1(self):
    ...
    out_s = self.model_s(
        ...
        self.node_types,
        self.adjs_pt,
        self.archs[self.args.dataset]["source"][0],
        self.archs[self.args.dataset]["source"][1],
        self.args.gpu,
    )
    out_t = self.model_t(
        self.node_feats,
        self.node_types,
        self.adjs_pt,
        self.archs[self.args.dataset]["target"][0],
        self.archs[self.args.dataset]["target"][1],
        self.args.gpu,
    )
    loss = -torch.mean(
        F.logsigmoid(
            torch.mul(
                out_s[self.train_pos[0].tolist()], out_t[self.train_pos[1].tolist()]
            ).sum(dim=-1)
        )
        + F.logsigmoid(
            -torch.mul(
                out_s[self.train_neg[0].tolist()], out_t[self.train_neg[1].tolist()]
            ).sum(dim=-1)
        )
    )
    loss.backward()
...
def infer(self):
    ...
    pos_val_prod = torch.mul(
        out_s[self.val_pos[0].tolist()], out_t[self.val_pos[1].tolist()]
    ).sum(dim=-1)
    neg_val_prod = torch.mul(
        out_s[self.val_neg[0].tolist()], out_t[self.val_neg[1].tolist()]
    ).sum(dim=-1)
    ...
    y_pred_val = np.concatenate(
        (
            torch.sigmoid(pos_val_prod).cpu().numpy(),
            torch.sigmoid(neg_val_prod).cpu().numpy(),
        )
    )
    auc_val = roc_auc_score(y_true_val, y_pred_val)
    # * test performance
    pos_test_prod = torch.mul(
        out_s[self.test_pos[0].tolist()], out_t[self.test_pos[1].tolist()]
    ).sum(dim=-1)
    neg_test_prod = torch.mul(
        out_s[self.test_neg[0].tolist()], out_t[self.test_neg[1].tolist()]
    ).sum(dim=-1)

    y_true_test = np.zeros(
        (len(self.test_pos[0]) + len(self.test_neg[0])), dtype=np.long
    )
    y_true_test[: len(self.test_pos[0])] = 1
    y_pred_test = np.concatenate(
        (
            torch.sigmoid(pos_test_prod).cpu().numpy(),
            torch.sigmoid(neg_test_prod).cpu().numpy(),
        )
    )
...
def train(self):
    ...
    for epoch in range(self.max_epoch):
        ...
        logging.info(
            "Epoch {}; Train err {}; Val err {}; Val auc {}".format(
                epoch + 1, train_loss, val_loss, auc_val
            )
        )
        if best_val is None or auc_val > best_val:
            ...
            anchor = epoch + 1
    logging.info(
        "Best val auc {} at epoch {}; Test auc {}".format(best_val, anchor, final)
    )
...
def search(self):
    ...
    model_s = self.args.search_model(
        self.in_dims,
        self.args.attn_dim,
        len(self.serach_adjs_pt),
        [self.args.search_steps_s],
        self.cstr_source[self.args.dataset],
    )
    model_t = self.args.search_model(
        self.in_dims,
        self.args.attn_dim,
        len(self.serach_adjs_pt),
        [self.args.search_steps_t],
        self.cstr_target[self.args.dataset],
    )
    ...
    optimizer_a = torch.optim.Adam(
        model_s.alphas() + model_t.alphas(), lr=self.args.search_alr
    )

    eps = self.args.search_eps
    for epoch in range(self.args.search_epochs):
        min_error = 100
        train_error, val_error = serach_train(
            self, model_s, model_t, optimizer_w, optimizer_a, eps
        )
        logging.info(
            "Epoch {}; Train err {}; Val err {}; Source arch {}; Target arch {}".format(
                epoch + 1, train_error, val_error, model_s.parse(), model_t.parse()
            )
        )
        if val_error < min_error:
            self.archs = {
                "amazon": {"source": model_s.parse(), "target": model_t.parse()},
            }
            min_error = val_error
        eps = eps * self.args.search_decay


def serach_train(self, model_s, model_t, optimizer_w, optimizer_a, eps):
    idxes_seq_s, idxes_res_s = model_s.sample(eps)
    idxes_seq_t, idxes_res_t = model_t.sample(eps)

    optimizer_w.zero_grad()
    out_s = model_s(
        self.node_feats, self.node_types, self.serach_adjs_pt, idxes_seq_s, idxes_res_s
    )
    out_t = model_t(
        self.node_feats, self.node_types, self.serach_adjs_pt, idxes_seq_t, idxes_res_t
    )
    loss_w = -torch.mean(
        F.logsigmoid(
            torch.mul(
