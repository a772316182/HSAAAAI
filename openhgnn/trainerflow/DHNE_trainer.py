...
class DHNE_trainer(BaseFlow):
    ...
    def collate_fn(self, batches):
        ...
        return [
            torch.concat(encode1).type(torch.float32),
            torch.concat(encode2).type(torch.float32),
            torch.concat(encode3).type(torch.float32),
        ], torch.concat(labels)
    ...
    def train(self):
        ...
        print(
            f"Epoch: {max_auc_epoch:03d}, train loss: {max_auc_epoch_loss:.4f}. "
            + "encode_0:{encode_0},encode_1:{encode_1},encode_2:{encode_2},label:{label},auc_micro:{auc_micro}".format(
