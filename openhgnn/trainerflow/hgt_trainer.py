...
class HGTTrainer(BaseFlow):
    ...
    def preprocess(self):
        if self.args.mini_batch_flag:
            N_SAMPLE_NODES_PER_TYPE = (
                1280  # number of nodes to sample per node type per sampler step
            )
    ...
    def evaluate(self, split=None):
        ...
        with torch.no_grad():
            sum_preds = torch.zeros(
                self.hg.num_nodes(self.category), self.num_classes
            ).to(self.device)
            ...
            if mask is not None:
                ...
                metric = self.evaluator(
                    self.labels[mask].to("cpu"), final_preds[mask].to("cpu")
                )
                return metric, loss
            else:
                masks = {
                ...
                losses = {
                    key: self.loss_fn(avg_preds[mask], self.labels[mask].squeeze())
                    for key, mask in masks.items()
                }
                print(
                    "Train:",
    ...
    def _test_step(self, split=None, logits=None):
        ...
        with torch.no_grad():
            ...
            if mask is not None:
                ...
                metric = self.evaluator(
                    self.labels[mask].to("cpu"), logits[mask].argmax(dim=1).to("cpu")
                )
                return metric, loss
            else:
                masks = {
                ...
                metrics = {
                    key: self.evaluator(
                        self.labels[mask].to("cpu"),
                        logits[mask].argmax(dim=1).to("cpu"),
                    )
                    for key, mask in masks.items()
                }
                losses = {
