...
class GraphSampler:
    ...
    def sample_graph_for_dis(self):
        ...
        pos_hg = dgl.heterograph(
            pos_dict, {nt: len(self.hg_dict[nt].keys()) for nt in self.hg_dict.keys()}
        )
        neg_hg1 = dgl.heterograph(
            neg_dict1, {nt: len(self.hg_dict[nt].keys()) for nt in self.hg_dict.keys()}
        )
        neg_hg2 = dgl.heterograph(
            pos_dict, {nt: len(self.hg_dict[nt].keys()) for nt in self.hg_dict.keys()}
        )
    ...
    def sample_graph_for_gen(self):
        ...
        for nt in self.hg_dict.keys():
            for src in self.hg_dict[nt].keys():
                for i in range(self.k):
                    ...
                    dst = self.hg_dict[nt][src][et][
                        random.randint(0, len(self.hg_dict[nt][src][et]) - 1)
                    ]
                    if et not in d:
                        d[et] = ([src], [dst])
                    else:
        ...
        return dgl.heterograph(
            d, {nt: len(self.hg_dict[nt].keys()) for nt in self.hg_dict.keys()}
        )
...
class HeGANTrainer(BaseFlow):
    ...
    def __init__(self, args):
        ...
        self.optim_dis = torch.optim.Adam(
            ...
            lr=args.lr_dis,
            weight_decay=args.wd_dis,
        )
        self.optim_gen = torch.optim.Adam(
            self.model.generator.parameters(), lr=args.lr_gen, weight_decay=args.wd_gen
        )
    ...
    def train(self):
        ...
        for epoch in epoch_iter:
            ...
            print(
                "discriminator:\n\tloss:{:.4f}\n\tmicro_f1: {:.4f},\n\tmacro_f1: {:.4f}".format(
                    dis_loss, dis_score[0], dis_score[1]
                )
            )
            print(
                "generator:\n\tloss:{:.4f}\n\tmicro_f1: {:.4f},\n\tmacro_f1: {:.4f}".format(
                    gen_loss, gen_score[0], gen_score[1]
                )
            )
    ...
    def _full_train_step(self):
        ...
        for _ in range(self.args.epoch_dis):
            ...
            noise_emb = {
                et: torch.tensor(
                    np.random.normal(
                        0.0,
                        self.args.sigma,
                        (pos_hg2.num_edges(et), self.args.emb_size),
                    ).astype("float32")
                ).to(self.device)
            ...
            generate_neighbor_emb = self.model.generator.generate_neighbor_emb(
                pos_hg2, noise_emb
            )
            pos_score, neg_score1, neg_score2 = self.model.discriminator(
                pos_hg, pos_hg1, pos_hg2, generate_neighbor_emb
            )
        ...
        for _ in range(self.args.epoch_gen):
            ...
            noise_emb = {
                et: torch.tensor(
                    np.random.normal(
                        0.0, self.args.sigma, (gen_hg.num_edges(et), self.args.emb_size)
                    ).astype("float32")
                ).to(self.device)
                for et in gen_hg.canonical_etypes
            }
            gen_hg = gen_hg.to(self.device)
            score = self.model.generator(
                gen_hg, dis_node_emb, dis_relation_matrix, noise_emb
            )
            gen_loss = (
                -torch.mean(F.logsigmoid(score)) * (1 - self.label_smooth)
