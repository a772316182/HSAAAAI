...
class HGCLtrainer(BaseFlow):
    def __init__(self, args):
        ...
        self.model = (
            ...
            .build_model_from_args(args=self.args, hg=self.hg)
            .to(self.device)
        )
        self.opt = optim.Adam(self.model.parameters(), lr=self.args.lr)

        trainMat = self.hg.adj_external(
            etype=("user", "interact_train", "item"), scipy_fmt="coo"
        )
        testMat = self.hg.adj_external(
            etype=("user", "interact_test", "item"), scipy_fmt="coo"
        )
        ...
        train_data = np.hstack(
            (train_u.reshape(-1, 1), train_v.reshape(-1, 1))
        ).tolist()
        ...
        self.train_loader = dataloader.DataLoader(
            train_dataset, batch_size=self.args.batch, shuffle=True, num_workers=0
        )
        self.test_loader = dataloader.DataLoader(
            test_dataset, batch_size=1024 * 1000, shuffle=False, num_workers=0
        )
        self.train_losses = []
        self.test_hr = []
        self.test_ndcg = []
    ...
    def _mini_train_step(self):
        ...
        for user, item_i, item_j in self.train_loader:
            ...
            (
                self.userEmbed,
                self.itemEmbed,
                self.ui_userEmbedall,
                self.ui_itemEmbedall,
                self.ui_userEmbed,
                self.ui_itemEmbed,
                metaregloss,
            ) = self.model(self.istrain, userindex, itemindex, norm=1)
            ...
            ssl_loss = (
                self.args.ssl_ureg * ssl_loss_user + self.args.ssl_ireg * ssl_loss_item
            )

            # prediction
            pred_pos, pred_neg = self.predictModel(
                self.ui_userEmbedall[user],
                self.ui_itemEmbedall[item_i],
                self.ui_itemEmbedall[item_j],
            )
            bpr_loss = -nn.LogSigmoid()(pred_pos - pred_neg).sum()
            epoch_loss += bpr_loss.item()
            regLoss = (
                t.norm(self.ui_userEmbedall[user]) ** 2
                + t.norm(self.ui_itemEmbedall[item_i]) ** 2
                + t.norm(self.ui_itemEmbedall[item_j]) ** 2
            )
            loss = (
                ((bpr_loss + regLoss * self.args.reg) / self.args.batch)
                + ssl_loss * self.args.ssl_beta
                + metaregloss * self.args.metareg
            )
    ...
    def test(self):
        ...
        with t.no_grad():
            ...
            self.istrain = False
            _, _, self.ui_userEmbed, self.ui_itemEmbed, _, _, _ = self.model(
                self.istrain, uid, iid, norm=1
            )
            for test_u, test_i in self.test_loader:
                test_u = test_u.long().cuda()
                test_i = test_i.long().cuda()
                pred = self.predictModel(
                    self.ui_userEmbed[test_u],
                    self.ui_itemEmbed[test_i],
                    None,
                    isTest=True,
                )
    ...
    def train(self):
        ...
        print("*****************************")
        self.log(
            "best epoch = %d, HR= %.4f, NDCG=%.4f" % (best_epoch, best_hr, best_ndcg)
        )
        print("*****************************")
        print(self.args)
