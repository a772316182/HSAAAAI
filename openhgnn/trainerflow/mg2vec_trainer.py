...
class Mg2vecTrainer(BaseFlow):
    def __init__(self, args):
        ...
        self.model = None
        self.embeddings_file_path = os.path.join(
            self.args.output_dir, self.args.dataset + "_mg2vec_embeddings.npy"
        )
        self.embeddings_file_path2 = os.path.join(
            self.args.output_dir, self.args.dataset + "_mg2vec_embeddings.txt"
        )
    ...
    def preprocess(self):
        ...
        self.mg2vec_sampler = mg2vec_sampler.Mg2vecSampler(
            input_file, block_size, self.args.alpha
        )
    ...
    def train(self):
        ...
        metric = {
            "test": self.task.downstream_evaluate(
                logits=self.get_edge_embed(emb=emb_dict), evaluation_metric="acc_f1"
            )
        }
    ...
    def load_embeddings(self):
        if not self.load_trained_embeddings or not os.path.exists(
    ...
    def train_embeddings(self):
        ...
        self.model.save_embedding(
            self.mg2vec_sampler.data.node_reverse_dict, self.embeddings_file_path2
        )
    ...
    def get_edge_embed(self, emb):
        ...
        for i in range(len(u)):
            edge_embed.append(
                np.hstack([emb[int(core1_dict[u[i]])], emb[int(core2_dict[v[i]])]])
            )
