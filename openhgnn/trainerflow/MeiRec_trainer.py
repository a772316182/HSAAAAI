...
class MeiRECTrainer(BaseFlow):
    ...
    def __init__(self, args):
        ...
        self.model = build_model(self.model_name).build_model_from_args(self.args).model
            ...
            if epoch % 3 == 0:
                for i, data in enumerate(pbar):
                    ...
                    if i != len(train_loader) - 1:
                        output, train_loss, train_auc, train_acc = self._train_step(
                            data
                        )
                        # record bar
                        ...
                        for j, data_test in enumerate(test_loader):
                            output, test_loss, test_auc, test_acc = self.eval_model(
                                data_test
                            )
                            pr.append(output)
    ...
    def auc(self, labels, preds, num_thresholds=200):  # tensorflow1.0_auc
        ...
        values = self._confusion_matrix_at_thresholds(labels, preds, thresholds)
        ...
        auc_value = compute_auc(values["tp"], values["fn"], values["tn"], values["fp"])
