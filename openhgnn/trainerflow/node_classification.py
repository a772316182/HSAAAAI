...
class NodeClassification(BaseFlow):
    ...
    def __init__(self, args):
        ...
        if self.args.mini_batch_flag:
            ...
            if self.train_idx is not None:
                if self.to_homo_flag:
                    loader_train_idx = to_homo_idx(
                        self.hg.ntypes,
                        self.num_nodes_dict,
                        {self.category: self.train_idx},
                    ).to(self.device)
                else:
            ...
            if self.train_idx is not None:
                if self.to_homo_flag:
                    loader_val_idx = to_homo_idx(
                        self.hg.ntypes,
                        self.num_nodes_dict,
                        {self.category: self.val_idx},
                    ).to(self.device)
                else:
            ...
            if self.args.test_flag:
                if self.test_idx is not None:
                    if self.to_homo_flag:
                        loader_test_idx = to_homo_idx(
                            self.hg.ntypes,
                            self.num_nodes_dict,
                            {self.category: self.test_idx},
                        ).to(self.device)
                    else:
            ...
            if self.args.prediction_flag:
                if self.pred_idx is not None:
                    if self.to_homo_flag:
                        loader_pred_idx = to_homo_idx(
                            self.hg.ntypes,
                            self.num_nodes_dict,
                            {self.category: self.pred_idx},
                        ).to(self.device)
                    else:
    ...
    def preprocess(self):
        ...
        if self.args.model == "GTN":
            if (
                hasattr(self.args, "adaptive_lr_flag")
                and self.args.adaptive_lr_flag == True
            ):
            ...
            if (
                ...
                self.optimizer = torch.optim.Adam(
                    [
                        {"params": self.model.HSAF.HLHIA_layer.gcn_list.parameters()},
        ...
        for epoch in epoch_iter:
            ...
            if epoch % self.evaluate_interval == 0:
                ...
                self.writer.add_scalars(
                    "loss", {"train": train_loss, "valid": val_loss}, global_step=epoch
                )
                for mode in modes:
                    self.writer.add_scalars(
                        f"metric_{mode}", metric_dict[mode], global_step=epoch
                    )
