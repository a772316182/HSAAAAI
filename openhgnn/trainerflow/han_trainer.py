...
class HANNodeClassification(BaseFlow):
    ...
    def __init__(self, args):
        ...
        if (
            not hasattr(self.task.dataset, "out_dim")
            or args.out_dim != self.num_classes
        ):
        ...
        if self.args.mini_batch_flag:
            sampler = HANSampler(
                g=self.hg,
                seed_ntypes=[self.category],
                meta_paths_dict=self.args.meta_paths_dict,
                num_neighbors=20,
            )
            ...
            if self.valid_idx is not None:
                self.val_loader = dgl.dataloading.DataLoader(
                    self.hg,
                    {self.category: self.valid_idx.to(self.device)},
                    sampler,
                    batch_size=self.args.batch_size,
                    device=self.device,
                    shuffle=True,
    ...
    def train(self):
        ...
        for k, v in self.model.input_feature.h_dict.items():
            self.model.input_feature.h_dict[k] = v.to(self.device)
        ...
        if self.args.test_flag:
            if self.args.dataset[:4] == "HGBn":
                ...
                self.logger.train_info(
                    "[Test Info]" + self.logger.metric2str(metric_dict)
                )
                self.model.eval()
                with torch.no_grad():
                    h_dict = self.model.input_feature()
                    logits = self.model(self.hg, h_dict)[self.category]
                    self.task.dataset.save_results(
                        logits=logits, file_path=self.args.HGB_results_path
                    )
    ...
    def _full_train_step(self):
        ...
        logits = self.model(self.hg, h_dict)[self.category]
        loss = self.loss_fn(
            logits[self.train_idx], torch.squeeze(self.labels[self.train_idx])
        )
    ...
    def _mini_train_step(
        ...
        for i, (
            ntype_mp_name_input_nodes_dict,
            seeds,
            ntype_mp_name_block_dict,
        ) in enumerate(loader_tqdm):
            ...
            for meta_path_name, input_nodes in mp_name_input_nodes_dict.items():
                emb_dict[meta_path_name] = self.model.input_feature.forward_nodes(
                    {self.category: input_nodes}
                )
    ...
    def _full_test_step(self, modes, logits=None):
        ...
        with torch.no_grad():
            ...
            loss_dict = {
                key: self.loss_fn(logits[mask], torch.squeeze(self.labels[mask])).item()
        ...
        with torch.no_grad():
            ...
            for mode in modes:
                ...
                for i, (
                    ...
                    seeds = seeds[self.category]
                    mp_name_input_nodes_dict = ntype_mp_name_input_nodes_dict[
                        self.category
                    ]
                    emb_dict = {}
                    for meta_path_name, input_nodes in mp_name_input_nodes_dict.items():
                        emb_dict[meta_path_name] = (
                            self.model.input_feature.forward_nodes(
                                {self.category: input_nodes}
                            )
                        )
                    emb_dict = {self.category: emb_dict}
                ...
                metric_dict[mode] = evaluator(
                    y_trues, y_predicts.argmax(dim=1).to("cpu")
                )
                loss_dict[mode] = loss
        return metric_dict, loss_dict
...
class HANLinkPrediction(BaseFlow):
    ...
    def __init__(self, args):
        ...
        if hasattr(self.args, "flag_add_reverse_edges") or self.args.dataset in [
            "ohgbl-MTWM",
            "ohgbl-yelp1",
            "ohgbl-yelp2",
        ]:
        ...
        self.model = (
            build_model(self.model)
            .build_model_from_args(self.args, self.train_hg)
            .to(self.device)
        )
        ...
        if self.args.score_fn == "distmult":
            ...
            """
            self.r_embedding = nn.ParameterDict(
                {
                    etype[1]: nn.Parameter(th.Tensor(1, self.args.out_dim))
                    for etype in self.train_hg.canonical_etypes
                }
            ).to(self.device)
        ...
        self.positive_graph = self.train_hg.edge_type_subgraph(self.target_link).to(
            self.device
        )
        if self.args.mini_batch_flag:
            ...
            sampler = dgl.dataloading.as_edge_prediction_sampler(
                sampler=sampler, negative_sampler=negative_sampler
            )
        ...
        for epoch in tqdm(range(self.max_epoch)):
            ...
            if epoch % self.evaluate_interval == 0:
                ...
                self.logger.train_info(
                    ...
                    + self.logger.metric2str(val_metric)
                )
        ...
        if self.args.test_flag:
            if self.args.dataset in ["HGBl-amazon", "HGBl-LastFM", "HGBl-PubMed"]:
                ...
                with torch.no_grad():
                    ...
                    score = th.sigmoid(
                        self.task.ScorePredictor(
                            self.task.test_hg, embedding, self.r_embedding
                        )
                    )
                    self.task.dataset.save_results(
                        hg=self.task.test_hg,
                        score=score,
    ...
    def construct_negative_graph(self, hg):
        e_dict = {
            etype: hg.edges(etype=etype, form="eid") for etype in hg.canonical_etypes
        }
        neg_srcdst = self.negative_sampler(hg, e_dict)
        neg_pair_graph = dgl.heterograph(
            neg_srcdst, {ntype: hg.number_of_nodes(ntype) for ntype in hg.ntypes}
        )
    ...
    def _mini_train_step(
        ...
        for (
            ...
            positive_graph = positive_graph.edge_type_subgraph(self.target_link).to(
                self.device
            )
            negative_graph = negative_graph.edge_type_subgraph(self.target_link).to(
                self.device
            )
            emb_dict = {}
            for (
                ntype,
                mp_name_input_nodes_dict,
            ) in ntype_mp_name_input_nodes_dict.items():
                mp_name_emb_dict = {}
                for meta_path_name, input_nodes in mp_name_input_nodes_dict.items():
                    mp_name_emb_dict[meta_path_name] = (
                        self.model.input_feature.forward_nodes({ntype: input_nodes})
                    )
    ...
    def loss_calculation(self, positive_graph, negative_graph, embedding):
        ...
        loss = F.binary_cross_entropy_with_logits(
            th.cat((p_score, n_score)), th.cat((p_label, n_label))
        )
        return loss
    ...
    def _mini_test_step(self, split=None):
        ...
        with th.no_grad():
            ntypes = get_ntypes_from_canonical_etypes(self.target_link)
            embedding = self._mini_embedding(
                model=self.model,
                fanouts=[20],
                g=self.train_hg,
    ...
    def _mini_prediction_step(self):
        ...
        with th.no_grad():
            ...
            embedding = self._mini_embedding(
                model=self.model,
                fanouts=[-1] * self.args.num_layers,
                g=self.train_hg,
                device=self.args.device,
                dim=self.model.out_dim,
                ntypes=ntypes,
    ...
    def _mini_embedding(self, model, fanouts, g, device, dim, ntypes, batch_size):
        ...
        with th.no_grad():
            ...
            indices = {
                ntype: torch.arange(g.num_nodes(ntype)).to(device) for ntype in ntypes
            }
            embedding = {
                ntype: torch.zeros(g.num_nodes(ntype), dim).to(device)
                for ntype in ntypes
            }
            dataloader = dgl.dataloading.DataLoader(
                g, indices, sampler, device=device, batch_size=batch_size
            )
