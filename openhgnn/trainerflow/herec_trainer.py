...
class HERecTrainer(BaseFlow):
    def __init__(self, args):
        ...
        self.embeddings_file_path = os.path.join(
            ...
            + "_"
            + self.args.meta_path_key
            + "_herec_embeddings.npy",
        )
    ...
    def preprocess(self):
        ...
        self.random_walk_sampler = random_walk_sampler.RandomWalkSampler(
            g=g.to("cpu"),
        ...
        if self.args.task == "node_classification":
            metric = {
                "test": self.task.downstream_evaluate(
                    logits=emb, evaluation_metric="f1_lr"
                )
            }
