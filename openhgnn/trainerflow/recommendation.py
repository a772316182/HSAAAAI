...
class Recommendation(BaseFlow):
    ...
    def train(self):
        ...
        print(
            f"Test Recall@K = {test_metric_dic['recall']: .4f}, NDCG@K = {test_metric_dic['ndcg']: .4f}"
        )
    ...
    def loss_calculation(self, positive_graph, negative_graph, embedding):
        p_score = self.ScorePredictor(positive_graph, embedding).repeat_interleave(
            self.num_neg
        )
    ...
    def _full_train_step(self):
        ...
        loss = self.loss_calculation(
            self.positive_graph, self.negative_graph, embedding
        )
    ...
    def _test_step(self, split=None, logits=None):
        ...
        with th.no_grad():
            ...
            score_matrix = (
                (embedding[self.user_name] @ embedding[self.item_name].T)
                .detach()
                .cpu()
                .numpy()
            )
            ...
            train_u, train_i = (
                self.positive_graph.edges(etype=self.target_link)[0].cpu().numpy(),
                self.positive_graph.edges(etype=self.target_link)[1].cpu().numpy(),
            )
            ...
            pred_list = ind[
                np.arange(len(score_matrix))[:, None], arr_ind_argsort
            ]  # (num_uses, k)
...
def recall_at_k(pred_list, test_graph, k, user_name, target_link):
    ...
    for user in range(test_graph.num_nodes(user_name)):
        test_items_set = set(
            test_graph.successors(user, etype=target_link).cpu().numpy()
        )
...
def compute_DCG(l):
    ...
    if l.size:
        return np.sum(
            np.subtract(np.power(2, l), 1) / np.log2(np.arange(2, l.size + 2))
        )
    else:
        return 0.0
