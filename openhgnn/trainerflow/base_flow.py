...
class BaseFlow(ABC):
    ...
    def __init__(self, args):
        ...
        if hasattr(args, "model_path"):
            ...
            self._checkpoint = os.path.join(
                args._checkpoint, f"{args.model_name}_{args.dataset_name}.pt"
            )
        else:
        ...
        if not hasattr(args, "HGB_results_path") and args.dataset_name[:3] == "HGB":
            args.HGB_results_path = os.path.join(
                args.output_dir,
                "{}_{}_{}.txt".format(
                    args.model_name, args.dataset_name[5:], args.seed
                ),
            )
    ...
    def feature_preprocess(self, act):
        ...
        if self.hg.ndata.get("h", {}) == {} or self.args.feat == 2:
            if self.hg.ndata.get("h", {}) == {}:
                self.logger.feature_info(
                    "Assign embedding as features, because hg.ndata is empty."
                )
            else:
                self.logger.feature_info("feat2, drop features!")
                self.hg.ndata.pop("h")
            self.input_feature = HeteroFeature(
                {}, get_nodes_dict(self.hg), self.args.hidden_dim, act=act
            ).to(self.device)
            ...
            if self.args.task != "node_classification":
                self.logger.feature_info(
                    "'feat 1' is only for node classification task, set feat 0!"
                )
                self.input_feature = self.init_feature(act)
            else:
                ...
                self.input_feature = HeteroFeature(
                    {self.category: h_dict[self.category]},
                    get_nodes_dict(self.hg),
    ...
    def init_feature(self, act):
        ...
        if isinstance(self.hg.ndata["h"], dict):
            ...
            input_feature = HeteroFeature(
                {self.hg.ntypes[0]: self.hg.ndata["h"]},
                get_nodes_dict(self.hg),
                self.args.hidden_dim,
                act=act,
            ).to(self.device)
    ...
    def load_from_pretrained(self):
        if (
            hasattr(self.args, "load_from_pretrained")
            and self.args.load_from_pretrained
        ):
            try:
                ck_pt = torch.load(self._checkpoint)
                self.model.load_state_dict(ck_pt)
                self.logger.info(
                    "[Load Model] Load model from pretrained model:" + self._checkpoint
                )
            except FileNotFoundError:
                self.logger.info(
                    "[Load Model] Do not load the model from pretrained, "
