...
class NodeClassificationAC(BaseFlow):
    ...
    def __init__(self, args):
        ...
        if self.args.mini_batch_flag:
            ...
            self.loader = dgl.dataloading.NodeDataLoader(
                self.hg.to("cpu"),
                {self.category: self.train_idx.to("cpu")},
                sampler,
    ...
    def preprocess(self):
        ...
        self.feat_keep_idx, self.feat_drop_idx = train_test_split(
            np.arange(self.in_dim[self.args.src_node_type]),
    ...
    def train(self):
        ...
        for epoch in epoch_iter:
            ...
            if epoch % self.evaluate_interval == 0:
                ...
                printInfo(
                    self.metric, epoch, train_score, train_loss, val_score, val_loss
                )
    ...
    def _full_train_step(self):
        ...
        feat_src_re = self.hgnn_ac(
            self.adj[self.ntypes[self.args.src_node_type]][:, self.feat_keep_idx],
            self.hg.nodes[self.ntypes[self.args.src_node_type]].data["h"],
            self.hg.nodes[self.ntypes[self.args.src_node_type]].data["h"][
                self.feat_keep_idx
            ],
        ...
        with self.hg.local_scope():
            for i, opt in enumerate(list(self.args.feats_opt)):
                if opt == "1":
                    feat_ac = self.hgnn_ac(
                        self.adj[self.ntypes[i]].t(),
                        self.hg.nodes[self.ntypes[i]].data["h"],
                        self.hg.nodes[self.ntypes[self.args.src_node_type]].data["h"],
    ...
    def _full_test_step(self, mode=None, logits=None):
        ...
        with torch.no_grad():
            ...
            if mask is not None:
                ...
                metrics = {
                    key: self.task.evaluate(
                        (logits.cpu().numpy() > 0).astype(int)
                        if self.task.multi_label
                        else logits.to("cpu")
                    )
