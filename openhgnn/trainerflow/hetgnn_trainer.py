...
from ..sampler.HetGNN_sampler import (
...
class HetGNNTrainer(BaseFlow):
    ...
    def preprocess(self):
        ...
        if self.args.mini_batch_flag:
            if self.args.model == "HetGNN":
                ...
                self.het_graph = hetg.get_hetgnn_graph(
                    self.args.rw_length, self.args.rw_walks, self.args.rwr_prob
                ).to("cpu")

                batch_sampler = SkipGramBatchSampler(
                    self.hg, self.args.batch_size, self.args.window_size
                )
                neighbor_sampler = NeighborSampler(
                    self.het_graph,
                    self.hg.ntypes,
                    batch_sampler.num_nodes,
                    self.args.device,
                )
        ...
        for epoch in epoch_iter:
            ...
            if epoch % self.evaluate_interval == 0:
                ...
                self.logger.train_info(
                    ...
                    + self.logger.metric2str(metrics)
                )
                if early_stop:
    ...
    def _full_train_setp(self):
        ...
        loss = self.loss_calculation(
            self.ScorePredictor(self.hg, x), self.ScorePredictor(negative_graph, x)
        )
    ...
    def _mini_train_step(
        ...
        for batch_id in range(self.args.batches_per_epoch):
            ...
            loss = self.loss_calculation(
                self.ScorePredictor(positive_graph, x),
                self.ScorePredictor(negative_graph, x),
            )
            all_loss += loss.item()
            self.optimizer.zero_grad()
            loss.backward()
