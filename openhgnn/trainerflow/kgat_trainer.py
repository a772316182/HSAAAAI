...
class KGAT_Trainer(BaseFlow):
    ...
    def preprocess(self):
        ...
        user_ids_batches = [
            user_ids[i : i + self.args.test_batch_size]
            for i in range(0, len(user_ids), self.args.test_batch_size)
        ]
        self.user_ids_batches = [
            torch.LongTensor(d).to(self.device) for d in user_ids_batches
        ]
        ...
        for epoch in range(1, self.max_epoch + 1):
            ...
            for iter in range(1, n_cf_batch + 1):
                ...
                cf_batch_user, cf_batch_pos_item, cf_batch_neg_item = (
                    self.generate_cf_batch(self.train_user_dict)
                )
            ...
            for iter in range(1, n_kg_batch + 1):
                ...
                (
                    ...
                    kg_batch_pos_tail,
                    kg_batch_neg_tail,
                ) = self.generate_kg_batch(self.train_kg_dict)
            ...
            # evaluate cf
            time1 = time()
            _, precision, recall, ndcg = self.evaluate(
                self.model,
                self.g,
            ...
            best_recall, should_stop = self.early_stopping(
                recall_list, self.args.stopping_steps
            )
            if should_stop:
                break
            ...
            if recall_list.index(best_recall) == len(recall_list) - 1:
                torch.save(
                    ...
                    f"{self.args.output_dir}/{self.model_name}_{self.args.dataset_name[0:-5]}_{self.args.aggregation_type}.pth",
                )
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            for user_ids_batch in tqdm(user_ids_batches, desc="Evaluating Iteration"):
                ...
                precision_batch, recall_batch, ndcg_batch = self.calc_metrics_at_k(
                    cf_scores_batch,
                    train_user_dict,
                    test_user_dict,
                    user_ids_batch,
    ...
    def calc_metrics_at_k(
        self, cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, K
    ):
        """
        ...
        test_pos_item_binary = np.zeros(
            [len(user_ids), len(item_ids)], dtype=np.float32
        )
