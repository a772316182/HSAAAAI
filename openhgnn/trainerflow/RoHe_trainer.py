...
class RoHeTrainer(BaseFlow):
    ...
    def __init__(
        self, args
    ):  # 传递对象（如列表、字典、类实例等）作为参数时，实际上传递的是对象的引用
        ...
        if (
            ...
            or self.args.out_dim != self.num_classes
        ):
        ...
        self.raw = True
        self.raw_model = (
            build_model("HAN").build_model_from_args(self.args, self.hg).to(self.device)
        )
        self.model = (
    ...
    def train(self):
        ...
        input_feature = HeteroFeature(
            self.hg.ndata["h"], get_nodes_dict(self.hg), self.args.hidden_dim, act=None
        )
    ...
    def _full_test_step(self, modes, logits=None):
        ...
        with torch.no_grad():
            ...
            loss_dict = {
                key: self.loss_fn(logits[mask], self.labels[mask]).item()
    ...
    def get_transition(self, given_hete_adjs, metapath_info):
        ...
        for key in given_hete_adjs.keys():
            ...
            hete_adj_dict_tmp[key] = given_hete_adjs[key] / (
    ...
    def setup_settings(self):
        ...
        trans_adj_list = self.get_transition(
            self.hete_adjs, meta_paths_dict["acm_han_raw"]
        )
    ...
    def load_acm(self, remove_self_loop=False):
        ...
        hete_adjs = {"pa": p_vs_a, "ap": p_vs_a.T, "pf": p_vs_l, "fp": p_vs_l.T}
        return hete_adjs
    ...
    def generate_attacks_metric_dict(self, raw=False):
        ...
        tar_nodes_num = 100
        tar_idx = sorted(
            random.sample(range(self.hg.num_nodes(self.category)), tar_nodes_num)
        )
        if raw:
            print(
                "Note that:\n"
        ...
        with torch.no_grad():
            if raw:
                ...
                _, micro_f1, macro_f1 = self.score(
                    logits[tar_idx], self.labels[tar_idx]
                )
                print(
                    f"Raw-HAN in raw data:  Micro-F1: {micro_f1:.3f} Macro-F1: {macro_f1:.3f}"
                )
            h_dict = self.model.input_feature()
            logits = self.model(self.hg, h_dict)[self.category]
            _, micro_f1, macro_f1 = self.score(logits[tar_idx], self.labels[tar_idx])
            print(
                f"RoHe-HAN in raw data:  Micro-F1: {micro_f1:.3f} Macro-F1: {macro_f1:.3f}"
            )
        ...
        for items in modified_opt_iter:
            ...
            trans_adj_list = self.get_transition(
                mod_hete_adj_dict, meta_paths_dict["acm_han_raw"]
            )
            for i in range(len(self.model.mod_dict["paper"].layers)):
                self.model.mod_dict["paper"].layers[i].model.mods["PAP"].settings[
                    "TransM"
                ] = trans_adj_list[0]
                self.model.mod_dict["paper"].layers[i].model.mods["PFP"].settings[
                    "TransM"
                ] = trans_adj_list[1]
        ...
        if raw:
            ...
            _, raw_tar_micro_f1_atk, raw_tar_macro_f1_atk = self.score(
                raw_logits_adv, raw_labels_adv
            )
            print(
        ...
        print(
            f"RoHe-HAN in attacked data:  Micro-F1: {tar_micro_f1_atk:.3f} Macro-F1: {tar_macro_f1_atk:.3f}"
        )
