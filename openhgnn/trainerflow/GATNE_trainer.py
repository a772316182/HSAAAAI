...
class GATNE(BaseFlow):
    ...
    def preprocess(self):
        ...
        for etype in self.hg.etypes:
            nodes = torch.unique(bidirected_hg.edges(etype=etype)[0]).repeat(
                self.args.rw_walks
            )
        ...
        self.train_pairs = generate_pairs(
            all_walks, self.args.window_size, self.args.num_workers
        )
        self.neighbor_sampler = NeighborSampler(
            bidirected_hg, [self.args.neighbor_samples]
        )
        ...
        self.nsloss = NSLoss(self.hg.num_nodes(), self.args.neg_size, self.args.dim).to(
            self.device
        )
        self.optimizer = torch.optim.Adam(
            [{"params": self.model.parameters()}, {"params": self.nsloss.parameters()}],
    ...
    def _full_train_step(self):
        ...
        data_iter = tqdm(
            ...
            total=(len(self.train_pairs) + (self.args.batch_size - 1))
    ...
    def _full_test_step(self):
        ...
        final_model = dict(
            zip(
                self.hg.etypes,
                [
                    th.empty(self.hg.num_nodes(), self.args.dim)
                    for _ in range(len(self.hg.etypes))
                ],
            )
        )
        ...
        for i in tqdm(range(self.hg.num_nodes()), desc="Evaluating..."):
            ...
            train_types = (
                torch.tensor(list(range(len(self.hg.etypes))))
                .unsqueeze(1)
                .to(self.device)
            )  # [0, 1]
            pairs = torch.cat(
