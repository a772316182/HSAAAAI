            ...
            if dst_type == etype[2]:
                ...
                for i in idxs:
                    ...
                    if src_type in self.NS.keys():
                        src_idx = th.tensor(
                            [i for i in src_idx if i not in self.NS[src_type]]
                        )
                    if src_idx.shape[0] > 0:
        ...
        for i in range(self.num_steps):
            ...
            for src_type, p in B.n_types.items():
                ...
                if p.max() > 0:
                    ...
                    sampled_idx = th.multinomial(
                        prob[src_type], self.num_nodes_per_type, replacement=False
                    )
...
def HGT_preprocess4mag(hg, train_idx):
    ...
    hg2.update_all(
        fn.copy_u("feat", "m"), fn.mean("m", "feat"), etype="writes_inv"
    )  # author
    hg2.update_all(
    ...
    for ntype in hg2.ntypes:
        hg2.nodes[ntype].data["train_mask"] = torch.zeros(
            hg2.num_nodes(ntype), dtype=torch.bool
        )
        if ntype == "paper":
