...
class TransX_Sampler:
    def __init__(self, hg, args):
        ...
        self.src, self.rel, self.dst = self._random_select_triplets(
            self.src, self.rel, self.dst, self.num_edges
        )  # tensor
    ...
    def get_pos(self):
        if self.cur_idx + self.batch_size < self.num_edges:
            edge_idx = th.arange(
                self.cur_idx, self.cur_idx + self.batch_size, dtype=th.int64
            )
            shuffle = False
        else:
