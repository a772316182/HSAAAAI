...
class HDESampler:
    ...
    def compute_hde(self, args):
        ...
        self.data_A_train, self.data_B_train, self.data_y_train = self.batch_data(
            self.g_train, args
        )
        print("Computing HDE for validation set...")
        self.data_A_val, self.data_B_val, self.data_y_val = self.batch_data(
            self.g_val, args
        )
        print("Computing HDE for test set...")
        self.data_A_test, self.data_B_test, self.data_y_test = self.batch_data(
            self.g_test, args
        )
        ...
        return (
            ...
            self.data_B_train,
            self.data_y_train,
            self.val_batch_A_fea,
            self.val_batch_B_fea,
            self.val_batch_y,
            self.test_batch_A_fea,
            self.test_batch_B_fea,
            self.test_batch_y,
        )
    ...
    def batch_data(self, g, args):
        ...
        for bx in tqdm(edge):
            ...
            negA, negB = self.subgraph_sampling_with_DE_node_pair(
                g, [bx[0], neg_tmpB_id], args
            )
    ...
    def subgraph_sampling_with_DE_node_pair(self, G, node_pair, args):
        """
        ...
        A_fea_batch = self.gen_fea_batch(sub_G_for_AB, A, SPD_based_on_node_pair, args)
        B_fea_batch = self.gen_fea_batch(sub_G_for_AB, B, SPD_based_on_node_pair, args)
    ...
    def gen_fea_batch(self, G, root, fea_dict, args):
        ...
        fea_batch.append(np.asarray(a, dtype=np.float32).reshape(-1, self.num_fea))

        ns_1 = [
            list(
                np.random.choice(
                    list(G.neighbors(node)) + [node], args.num_neighbor, replace=True
                )
            )
        ...
        de_1 = [
            np.concatenate(
                [fea_dict[dest], np.asarray(self.type_encoder(dest))], axis=0
            )
            for dest in ns_1[0]
        ]
            ...
            for j in range(len(ns_2[0])):
                tmp.append(
                    np.concatenate(
                        [
                            fea_dict[ns_2[i][j]],
                            np.asarray(self.type_encoder(ns_2[i][j])),
                        ],
                        axis=0,
                    )
                )
    ...
    def dist_encoder(self, src, dest, G, args):
        ...
        one_hot_list = [
            np.eye(args.max_dist + 1, dtype=np.float64)[cnt[i]]
