...
class RandomWalkSampler(Dataset):
    def __init__(
        ...
        if metapath is not None:
            ...
            self.start_nodes = (
                list(range(self.g.num_nodes(start_ntype))) * self.rw_walks
            )
        else:
    ...
    def __generate_metapath(self):
        self.traces, self.trace_ntype = dgl.sampling.random_walk(
            g=self.g,
        ...
        self.traces_idx_of_all_types = (
            torch.index_select(
                torch.tensor(self.start_idx), 0, self.trace_ntype
            ).repeat(len(self.traces), 1)
            + self.traces
        )
