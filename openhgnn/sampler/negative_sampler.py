...
class NegativeSampler(object):
    def __init__(self, g, k):
        ...
        self.weights = {
            etype: g.in_degrees(etype=etype).float() ** 0.75 for etype in g.etypes
        }
        self.k = k
...
class Uniform_exclusive(_BaseNegativeSampler):
    ...
    def _generate(self, g, eids, canonical_etype):
        ...
        for i in range(len(eids)):
            ...
            dst = th.randint(
                low=0, high=g.number_of_nodes(canonical_etype[2]), size=(self.k,)
            )
            for d in range(len(dst)):
                while dst[d] in exp:
                    dst[d] = th.randint(
                        low=0, high=g.number_of_nodes(canonical_etype[2]), size=(1,)
                    )
