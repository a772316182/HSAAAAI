...
class SLiCESampler(object):
        ...
        for source in seed_nodes:
            ...
            for _ in range(self.num_walks_per_node):
                if self.walk_type == "bfs":
                    node_context = self.bfs_beam(
                        source, self.beam_width, self.max_num_edges
                    )
    ...
    def get_random_k_nbrs(self, nodeid, exclude_list, k):
        ...
        all_nbrs.extend(
            dgl.sampling.sample_neighbors(
                self.g, torch.tensor([nodeid]), -1, edge_dir="out", prob="label"
            )
            .edges(order="eid")[1]
            .tolist()
        )
        ...
        if len(all_nbrs) < k:
            all_nbrs.extend(
                dgl.sampling.sample_neighbors(
                    self.g, torch.tensor([nodeid]), -1, edge_dir="in", prob="label"
                )
                .edges(order="eid")[0]
                .tolist()
            )
    ...
    def random_chain(self, source, max_num_edges):
        ...
        for _ in range(max_num_edges):
            nbrs = self.get_random_k_nbrs(
                source, exclude_list=nodes_so_far, k=max_num_edges
            )
            nbr = -1
            while True:
                cursor = -1
                ...
                if nbr == -1:  # 如果没有邻居或者所有邻居均已被采样
                    ...
                    cursor -= 1
                    nbrs = self.get_random_k_nbrs(
                        source, exclude_list=nodes_so_far, k=1
                    )
                else:
                    break
            path.append(nbr)
        ...
        if option == "shortest":
            try:
                ...
                if len(path_node_list) <= max_seq_len:
                    ...
                    paths.append(
                        [source] + [target] + path_node_list[1 : max_seq_len - 1]
                    )
