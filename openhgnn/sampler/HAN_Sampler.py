...
class HANSampler(dgl.dataloading.Sampler):
    ...
    def __init__(self, g, seed_ntypes, meta_paths_dict, num_neighbors):
        ...
        for ntype, meta_paths_dict in self.ntype_meta_paths_dict.items():
            if len(meta_paths_dict) == 0:
                self.ntype_meta_paths_dict[ntype] = extract_metapaths(
                    ntype, g.canonical_etypes
                )
        ...
        for ntype, meta_paths_dict in self.ntype_meta_paths_dict.items():
            ...
            for meta_path_name, meta_path in meta_paths_dict.items():
                ...
                self.ntype_mp_name_sampler_dict[ntype][meta_path_name] = (
    ...
    def sample(
        self, g, seeds, exclude_eids=None
    ):  # exclude_eids is for compatibility with link prediction
        """sample method.
        ...
        for ntype, nid in seeds.items():
            ...
            for meta_path_name, sampler in self.ntype_mp_name_sampler_dict[
                ntype
            ].items():
