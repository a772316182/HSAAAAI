...
class MAGNN_sampler:
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(self.num_layers):
            mini_mp_inst = mini_mp_instance_sampler(
                seed_nodes=seed_nodes,
                mp_instances=self.mp_inst,
            ...
            for metapath in mini_mp_inst.keys():
                ...
                for i in range(0, len(metapath)):
                    if not metapath[i] in seed_nodes.keys():
                        ...
                        seed_nodes[metapath[i]] = np.concatenate(
                            (seed_nodes[metapath[i]], _mini_mp_inst[:, i]), axis=0
                        )
            for ntype in seed_nodes.keys():
                seed_nodes[ntype] = np.unique(seed_nodes[ntype])

        for meta, idx in mini_mp_inst.items():
            mini_mp_inst[meta] = np.unique(
                np.concatenate((idx, np.flip(idx, axis=1))), axis=0
            )
    ...
    def convert_mp_nids(old_metapath_dict, old_nids):
        ...
        for meta in old_metapath_dict.keys():
            for i, ntype in enumerate(meta):
                old_metapath_dict[meta][:, i] = np.array(
                    list(
                        map(
                            lambda x: np.argwhere(old_nids[ntype] == x)[0][0],
    ...
    for _batch in batch:
        ...
        for meta_type in meta_types:
            ...
            if meta_type in mini_mp_inst.keys():
                mini_mp_inst[meta_type] = np.concatenate(
                    (mini_mp_inst[meta_type], _batch[1][meta_type]), axis=0
                )
            else:
        ...
        for seed_ntype in seed_ntypes:
            ...
            if seed_ntype in seed_nodes.keys():
                seed_nodes[seed_ntype] = np.concatenate(
                    (seed_nodes[seed_ntype], _batch[2][seed_ntype]), axis=0
                )
            else:
