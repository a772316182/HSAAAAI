...
def test_get_center_context_negatives():
    rw_walks = 5
    hetero_sampler = RandomWalkSampler(
        g=hg,
        metapath=["follow", "view", "viewed-by"] * 3,
        rw_walks=rw_walks,
        window_size=3,
        neg_size=5,
    )
    ...
    homo_sampler = RandomWalkSampler(
        g=g, rw_length=10, rw_walks=rw_walks, window_size=3, neg_size=5
    )
