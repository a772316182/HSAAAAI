...
def HGNNLayer(
    gnn_type, rel_names, dim_in, dim_out, dropout, act, has_bn, has_l2norm, **kwargs
):
    return HeteroGeneralLayer(
        gnn_type, rel_names, dim_in, dim_out, dropout, act, has_bn, has_l2norm, **kwargs
    )
...
def GNNLayer(gnn_type, dim_in, dim_out, dropout, act, has_bn, has_l2norm, **kwargs):
    return GeneralLayer(
        gnn_type, dim_in, dim_out, dropout, act, has_bn, has_l2norm, **kwargs
    )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if num_layers == 1:
            ...
            self.f.append(
                GNNLayer(
                    gnn_type, d_in, dim_out, dropout, None, has_bn, has_l2norm, **kwargs
                )
            )
        self.act = act
        if stage_type == "skipsum":
    ...
    def __init__(
        ...
        **kwargs
        ...
        for i in range(num_layers):
            ...
            # else:
            layer = GNNLayer(
                gnn_type, d_in, dim_out, dropout, act, has_bn, has_l2norm, **kwargs
            )
            self.add_module("layer{}".format(i), layer)
        self.dim_out = dim_out
        self.has_l2norm = has_l2norm
