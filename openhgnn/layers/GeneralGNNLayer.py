    ...
    def __init__(
        ...
        **kwargs
        ...
        if kwargs.get("meta_paths") is not None:
            ...
            self.layer = homo_layer_dict[name](
                dim_in, dim_out, bias=not has_bn, **kwargs
            )
        layer_wrapper = []
        if has_bn:
...
class MultiLinearLayer(nn.Module):
    def __init__(
        self, linear_list, dropout, act=None, has_bn=True, has_l2norm=False, **kwargs
    ):
...
class GCNConv(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        self.model = dgl.nn.pytorch.GraphConv(
            dim_in, dim_out, norm="both", bias=bias, allow_zero_in_degree=True
        )
...
class SAGEConv(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        self.model = dgl.nn.pytorch.SAGEConv(
            dim_in, dim_out, aggregator_type="mean", bias=bias
        )
...
class GATConv(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        self.model = dgl.nn.pytorch.GATConv(
            dim_in,
            dim_out,
...
class GINConv(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        lin = nn.Sequential(
            nn.Linear(dim_in, dim_out, bias), nn.ReLU(), nn.Linear(dim_out, dim_out)
        )
...
class SimpleConv(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        self.model = SimpleHGNConv(
            dim_in,
            dim_in,
            int(dim_out / kwargs["num_heads"]),
            kwargs["num_heads"],
            kwargs["num_etypes"],
            beta=0.0,
        )
