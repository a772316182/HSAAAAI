...
class HeteroEmbedLayer(nn.Module):
    ...
    def __init__(
        self, n_nodes_dict, embed_size, embed_name="embed", activation=None, dropout=0.0
    ):
...
class multi_Linear(nn.Module):
    def __init__(self, linear_list, bias=False):
        ...
        for linear in linear_list:
            self.encoder[linear[0]] = nn.Linear(
                in_features=linear[1], out_features=linear[2], bias=bias
            )
...
class multi_2Linear(nn.Module):
    def __init__(self, linear_list, bias=False):
        ...
        for linear in linear_list:
            self.hidden_layer[linear[0]] = nn.Linear(
                in_features=linear[1], out_features=hidden_dim, bias=bias
            )
            self.output_layer[linear[0]] = nn.Linear(
                in_features=hidden_dim, out_features=linear[2], bias=bias
            )
