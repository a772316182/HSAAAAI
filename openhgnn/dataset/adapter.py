...
class AsNodeClassificationDataset(DGLDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(data, DGLDataset):
            ...
            self.dataset = None
            self.g = data
            assert name is not None, "Name is required when data is a graph."
        else:
        ...
        super().__init__(
            name + "-as-nodepred",
            hash_key=(self.split_ratio, target_ntype, name, "nodepred"),
            **kwargs,
        )
    ...
    def process(self):
        ...
        if self.prediction_ratio is None:
            if self.verbose:
                print(
                    "Prediction ratio is not provided, we will use the pred_mask from the original graph."
                )
        ...
        if self.multi_label is None:
            self.multi_label = (
                len(self.g.nodes[self.target_ntype].data[self.label_feat_name].shape)
                == 2
            )
        ...
        if self.num_classes is None:
            if self.multi_label:
                self.num_classes = (
                    self.g.nodes[self.target_ntype].data[self.label_feat_name].shape[1]
                )
            else:
                self.num_classes = len(
                    F.unique(self.g.nodes[self.target_ntype].data[self.label_feat_name])
                )
    ...
    def gene_pred_mask(self, ratio, ntype):
        idx_tensor = torch.where(
            self.g.nodes[ntype].data[self.label_mask_feat_name] == 0
        )[0]
    ...
    def gene_mask(self, ratio, ntype):
        ...
        if self.label_mask_feat_name is None:
            ...
            idx_tensor = torch.nonzero(
                self.g.nodes[ntype].data[self.label_mask_feat_name]
            ).squeeze(1)
        idx = idx_tensor.tolist()
        ...
        train_mask = utils.generate_mask_tensor(
            utils.idx2mask(idx[:n_train], len_nodes)
        )
        val_mask = utils.generate_mask_tensor(
            utils.idx2mask(idx[n_train : n_train + n_val], len_nodes)
        )
        test_mask = utils.generate_mask_tensor(
            utils.idx2mask(idx[n_train + n_val :], len_nodes)
        )
    ...
    def save(self):
        utils.save_graphs(
            os.path.join(self.save_path, "graph_{}.bin".format(self.hash)), [self.g]
        )
        with open(
            ...
            json.dump(
                {
                    ...
                    "label_mask_feat_name": self.label_mask_feat_name,
                },
                f,
            )
...
class AsLinkPredictionDataset(DGLDataset):
    ...
    def process(self):
        if self.split_ratio is None:
            for etype in self.target_link:
                for mask in ["train_mask", "val_mask", "test_mask"]:
                    assert (
                        mask in self.g.edges[etype].data
        ...
        if self.pred_edges is not None:
            self.pred_graph = dgl.heterograph(
                self.pred_edges,
        ...
        for i, etype in enumerate(self.target_link):
            ...
            train_graph = dgl.remove_edges(
                train_graph,
                th.cat(
                    (
                        self.pos_val_graph.edges[etype].data[dgl.EID],
            ...
            if self.target_link_r is not None:
                ...
                train_graph = dgl.remove_edges(
                    train_graph,
                    th.arange(train_graph.num_edges(reverse_etype)),
                    reverse_etype,
                )
                edges = train_graph.edges(etype=etype)
                train_graph = dgl.add_edges(
                    train_graph, edges[1], edges[0], etype=reverse_etype
                )
    ...
    def _get_pos_and_neg_graph(self, split):
        ...
        edges = {
            etype: th.nonzero(
                self.g.edges[etype].data["{}_mask".format(split)]
            ).squeeze()
            for etype in self.target_link
        }
        pos_graph = dgl.edge_subgraph(
            self.g, edges, relabel_nodes=False, store_ids=True
        )
        neg_edges = getattr(
            self.dataset, "neg_{}_edges".format(split), neg_sampler(self.g, edges)
        )
        neg_graph = dgl.heterograph(
            neg_edges, {ntype: pos_graph.num_nodes(ntype) for ntype in pos_graph.ntypes}
        )
