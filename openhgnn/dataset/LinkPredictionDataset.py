    ...
    def get_split(self, val_ratio=0.1, test_ratio=0.2):
        ...
        for i, etype in enumerate(self.target_link):
            ...
            if "train_mask" not in self.g.edges[etype].data:
                ...
                test_index = random_int[
                    int(num_edges * val_ratio) : int(
                        num_edges * (test_ratio + val_ratio)
                    )
                ]
                ...
                train_graph = dgl.remove_edges(
                    train_graph, th.cat((val_index, test_index)), etype
                )
                ...
                if self.target_link_r is None:
                    ...
                    train_graph = dgl.remove_edges(
                        train_graph,
                        th.arange(train_graph.num_edges(reverse_edge)),
                        reverse_edge,
                    )
                    edges = train_graph.edges(etype=etype)
                    train_graph = dgl.add_edges(
                        train_graph, edges[1], edges[0], etype=reverse_edge
                    )
...
class Test_LinkPrediction(LinkPredictionDataset):
    ...
    def preprocess(self):
        ...
        self.pos_test_graph = dgl.heterograph(
            {("user", "user-item", "item"): self.test_edge},
...
class HIN_LinkPrediction(LinkPredictionDataset):
    ...
    def load_HIN(self, dataset_name):
        ...
        if dataset_name == "academic4HetGNN":
            ...
            self.train_batch = self.load_link_pred(
                "./openhgnn/dataset/" + dataset_name + "/a_a_list_train.txt"
            )
            self.test_batch = self.load_link_pred(
                "./openhgnn/dataset/" + dataset_name + "/a_a_list_test.txt"
            )
            ...
            self.meta_paths_dict = {
                "UPU1": [
                    ("user", "user-buy-poi", "poi"),
                    ("poi", "user-buy-poi-rev", "user"),
                ],
                "UPU2": [
                    ("user", "user-click-poi", "poi"),
                    ("poi", "user-click-poi-rev", "user"),
                ],
                "USU": [
                    ("user", "user-buy-spu", "spu"),
                    ("spu", "user-buy-spu-rev", "user"),
                ],
                "UPSPU1": [
                    ("user", "user-buy-poi", "poi"),
                    ("poi", "poi-contain-spu", "spu"),
                    ("spu", "poi-contain-spu-rev", "poi"),
                    ("poi", "user-buy-poi-rev", "user"),
                ],
                "UPSPU2": [
                    ("user", "user-click-poi", "poi"),
...
class HGB_LinkPrediction(LinkPredictionDataset):
    ...
    def get_split(self):
        ...
        for i, etype in enumerate(self.target_link):
            ...
            random_int = th.randperm(len(train_index))[
                : int(len(train_index) * val_ratio)
            ]
            ...
            if self.target_link_r is None:
                ...
                train_graph = dgl.remove_edges(
                    train_graph, th.cat((val_index, test_index)), self.target_link_r[i]
                )
        self.out_ntypes = set(out_ntypes)
        val_graph = dgl.heterograph(
            val_edge_dict,
        ...
        with hg.local_scope():
            ...
            for etype in hg.canonical_etypes:
                ...
                edge_type_list.append(
                    th.full((src_id.shape[0],), self.test_edge_type[etype[1]])
                )
            ...
            with open(file_path, "w") as f:
                for l, r, edge_type, c in zip(
                    src_list, dst_list, edge_type_list, score
                ):
    ...
    def modify_size(self, eval_percent, dataset_type):
        if dataset_type == "valid":
            self.valid_triplets = th.tensor(
                random.sample(
                    self.valid_triplets.tolist(),
                    math.ceil(self.valid_triplets.shape[0] * eval_percent),
                )
            )
        elif dataset_type == "test":
            self.test_triplets = th.tensor(
                random.sample(
                    self.test_triplets.tolist(),
                    math.ceil(self.test_triplets.shape[0] * eval_percent),
                )
            )
