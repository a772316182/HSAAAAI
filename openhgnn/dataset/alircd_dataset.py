...
__all__ = [
    "AliRCDDataset",
    "AliRCDSmallDataset",
    "AliRCDSession1Dataset",
    "AliRCDSession2Dataset",
    "AliICDMDataset",
]
...
class AliRCDDataset(DGLBuiltinDataset):
    ...
    def __init__(
        self, session, raw_dir=None, force_reload=False, verbose=False, transform=None
    ):
        ...
        super(AliRCDDataset, self).__init__(
            name,
            url="https://s3.cn-north-1.amazonaws.com.cn/dgl-data/dataset/openhgnn/{}.zip".format(
                name
            ),
            raw_dir=raw_dir,
    ...
    def process(self):
        ...
        if self.load_labels:
            labels_path = os.path.join(
                self.save_path, "{}_train_labels.csv".format(self.name)
            )
            ...
            val_mask[
                label_idx[
                    int(split_ratio[0] * num_labels) : int(
                        (split_ratio[0] + split_ratio[1]) * num_labels
                    )
                ]
            ] = True
        ...
        test_idx_path = os.path.join(
            self.save_path, "{}_test_ids.csv".format(self.name)
        )
        ...
        if self.session == "ICDM":
            test_labels_path = os.path.join(
                self.save_path, "{}_test_labels.csv".format(self.name)
            )
        ...
        with open(node_file, "r") as rf:
            while True:
                ...
                if node_type == "item" and len(info[2]) == 0:
                    ...
                    node_embeds.setdefault(node_type, {})
                    node_embeds[node_type][node_id_v2] = np.array(
                        [x for x in info[2].split(":")], dtype=np.float32
                    )
                count += 1
                if count % 100000 == 0:
    ...
    def save(self):
        ...
        pkl.dump(
            self.item_map, open(os.path.join(self.save_path, "item_map.pkl"), "wb")
        )
        pkl.dump(
            self.rev_item_map,
            open(os.path.join(self.save_path, "rev_item_map.pkl"), "wb"),
        )

    def load(self):
        graph_path = os.path.join(self.save_path, "graph.bin")
        gs, _ = load_graphs(graph_path)
        self._item_map = pkl.load(
            open(os.path.join(self.save_path, "item_map.pkl"), "rb")
        )
        self._rev_item_map = pkl.load(
            open(os.path.join(self.save_path, "rev_item_map.pkl"), "rb")
        )
        self._g = gs[0]
        if self.verbose:
            print(self._g)
...
class AliRCDSmallDataset(AliRCDDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        session = "small"
        super(AliRCDSmallDataset, self).__init__(
            session,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
...
class AliRCDSession1Dataset(AliRCDDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        session = "session1"
        super(AliRCDSession1Dataset, self).__init__(
            session,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
...
class AliRCDSession2Dataset(AliRCDDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        session = "session2"
        super(AliRCDSession2Dataset, self).__init__(
            session,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
...
class AliICDMDataset(AliRCDDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        session = "ICDM"
        super(AliICDMDataset, self).__init__(
            session,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
