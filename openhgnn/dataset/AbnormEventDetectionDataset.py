...
class AbnormEventDetectionDataset(BaseDataset):
    ...
    def __init__(self, dataset_name, *args, **kwargs):
        ...
        self.g, self.center_type, self.context_type, self.event_label = self.get_graph(
            dataset_name
        )
    ...
    def get_batch(self, batch_size, shuffle: bool = True, device="cpu"):
        ...
        event_batch = self.get_batch_sample(
            batch_size, self.event_features, event_len, shuffle_list, device=device
        )
        neg_event_batch = self.get_batch_sample(
            batch_size, self.neg_event_features, event_len, shuffle_list, device=device
        )
        pos_event_batch = self.get_batch_sample(
            batch_size, self.pos_event_features, event_len, shuffle_list, device=device
        )
        neg_context_batch = self.get_batch_sample(
            batch_size,
            self.neg_context_features,
            event_len,
            shuffle_list,
            device=device,
        )
        neg_entity_batch = self.get_batch_sample(
            batch_size, self.neg_entity_features, event_len, shuffle_list, device=device
        )
        event_mask = self.get_batch_mask(
            batch_size, self.event_mask, event_len, shuffle_list, device=device
        )

        type_num = self.get_batch_type_num(
            batch_size, self.type_num, event_len, shuffle_list, device=device
        )
    ...
    def get_batch_type_num(
        self, batch_size, type_num: dict, event_len, shuffle_list, device="cpu"
    ):
        type_num_batch_list = []
        ...
        for i in range(type_num_batch_num):
            type_num_batch = dict()
            for key in type_num.keys():
                type_num_batch[key] = type_num_[key][
    ...
    def get_batch_mask(
        self, batch_size, mask: dict, event_len, shuffle_list, device="cpu"
    ):
        mask_batch_list = []
        ...
        if batch_size * mask_batch_num < event_len:
            mask_batch = dict()
            for key in mask.keys():
                mask_batch[key] = mask_[key][mask_batch_num * batch_size :, :].to(
                    device
                )
            mask_batch_list.append(mask_batch)
    ...
    def get_batch_sample(
        self, batch_size, event: dict, event_len, shuffle_list, device="cpu"
    ):
        event_batch_list = []
        ...
        if batch_size * event_batch_num < event_len:
            event_batch = dict()
            for key in event.keys():
                event_batch[key] = event_[key][event_batch_num * batch_size :, :, :].to(
                    device
                )
            event_batch_list.append(event_batch)
    ...
    def get_graph(self, name_dataset):
        if name_dataset == "aminer4AEHCL":
            ...
            if not os.path.exists(data_path):
                # download file
                download(
                    url="https://s3.cn-north-1.amazonaws.com.cn/dgl-data/dataset/aminer4aehcl.zip",
                    path="./openhgnn/dataset/",
                )
    ...
    def preprocess(self):
                ...
                for j in range(1000):
                    ...
                    while can_use is False:
                        ...
                        for key in events_context_type_node[i].keys():
                            if (
                                len(
                                    set(events_context_type_node[i][key])
                                    & set(events_context_type_node[num][key])
                                )
                                > 0
                            ):
                                can_use = False
                ...
                for node in tbar:
                    ...
                    for tp in relation_type[key]:
                        for node1 in all_type_set[tp]:
                            if node != node1 and (
                                node1 not in self.pos_node_set_dict[node[0]][node[1]]
                            ):
        ...
        self.max_type_features_len = (
            self.g.nodes[self.center_type].data["features"].shape[1]
        )
        for tp in self.context_type:
    ...
    def get_complete_events_features(self):
        ...
        for i in range(len(self.event_list)):
            ...
            for node in self_event:
                nd_list = random.sample(
                    self.neg_node_set_dict[node[0]][node[1]], self.neg_num
                )
                ndd_list = []
                for nd in nd_list:
                    ndd_list.append(nd)
            ...
            for key in type_context.keys():
                ...
                neg_nd_label = random.sample(self.type_label_node_list[key].keys(), 1)[
                    0
                ]
                neg_nd = random.sample(self.type_label_node_list[key][neg_nd_label], 1)[
                    0
                ]
                while neg_nd in type_context[key] or neg_nd_label == nd_label:
                    # print(key,": ","get neg node")
                    neg_nd_label = random.sample(
                        self.type_label_node_list[key].keys(), 1
                    )[0]
                    neg_nd = random.sample(
                        self.type_label_node_list[key][neg_nd_label], 1
                    )[0]
        ...
        event_features, self.event_mask, self.type_num = self.get_events_features(
            self_events
        )
    ...
    def get_events_features(self, events):
        ...
        for event in events:
            ...
            for key in event_features:
                mask[key] = [1.0] * len(event_features[key]) + [0.0] * (
                    self.type_max_num[key] - len(event_features[key])
                )
                if key != self.center_type:
                    type_num[key] = [self.context_type_num[key]] * len(
                        event_features[key]
                    ) + [0] * (self.type_max_num[key] - len(event_features[key]))
                event_features[key] += [[0.0] * self.max_type_features_len] * (
                    self.type_max_num[key] - len(event_features[key])
                )
