...
class GTNDataset(DGLBuiltinDataset):
    ...
    def __init__(
        self, name, raw_dir=None, force_reload=False, verbose=False, transform=None
    ):
    ...
    def process(self):
        ...
        if os.path.isfile(os.path.join(self.save_path, "graph.bin")):  # Has cache
            ...
            for i, split in enumerate(["train", "val", "test"]):
                ...
                g.nodes[target_ntype].data["{}_mask".format(split)] = th.from_numpy(
                    idx2mask(node, g.num_nodes(target_ntype))
                ).type(th.bool)
        ...
        self._num_classes = len(
            th.unique(self._g.nodes[self.target_ntype].data["label"])
        )
...
class DBLP4GTNDataset(GTNDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        name = "dblp4GTN"
        super(DBLP4GTNDataset, self).__init__(
            name,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
...
class ACM4GTNDataset(GTNDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        name = "acm4GTN"
        super(ACM4GTNDataset, self).__init__(
            name,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
...
class IMDB4GTNDataset(GTNDataset):
    def __init__(self, raw_dir=None, force_reload=False, verbose=False, transform=None):
        name = "imdb4GTN"
        super(IMDB4GTNDataset, self).__init__(
            name,
            raw_dir=raw_dir,
            force_reload=force_reload,
            verbose=verbose,
