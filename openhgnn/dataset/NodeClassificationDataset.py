...
class NodeClassificationDataset(BaseDataset):
    ...
    def get_split(self, validation=True):
        ...
        if "train_mask" not in self.g.nodes[self.category].data:
            self.logger.dataset_info(
                "The dataset has no train mask. "
            ...
            if validation:
                self.logger.dataset_info(
                    "Split train into train/valid with the ratio of 8:2 "
                )
...
class HIN_NodeClassification(NodeClassificationDataset):
    ...
    def load_HIN(self, name_dataset):
        if name_dataset == "demo_graph":
            ...
            self.meta_paths_dict = {
                "DCD": [
                    ("DISEASE", "CHEMICAL-in-DISEASE-rev", "CHEMICAL"),
                ...
                "DGD": [
                    ("DISEASE", "GENE-causing-DISEASE-rev", "GENE"),
                    ("GENE", "GENE-causing-DISEASE", "DISEASE"),
                ],
                "DSD": [
                    ("DISEASE", "SPECIES-with-DISEASE-rev", "SPECIES"),
            ...
            if name_dataset == "acm_han":
                ...
                return NotImplementedError(
                    "Unsupported dataset {}".format(name_dataset)
                )
...
class OHGB_NodeClassification(NodeClassificationDataset):
    def __init__(self, dataset_name, *args, **kwargs):
        ...
        if dataset_name == "ohgbn-Freebase":
            ...
            g = add_reverse_edges(g)
            self.meta_paths_dict = {
                "BB": [("BOOK", "BOOK-and-BOOK", "BOOK")],
                "BFB": [
                    ("BOOK", "BOOK-to-FILM", "FILM"),
                ...
                "BPB": [
                    ("BOOK", "PEOPLE-to-BOOK-rev", "PEOPLE"),
    ...
    def save_results(self, logits, file_path):
        ...
        if self.dataset_name == "HGBn-IMDB":
            ...
            for i in range(pred.shape[0]):
                label_list = [
                    str(j) for j in range(pred[i].shape[0]) if pred[i][j] == 1
                ]
...
class OGB_NodeClassification(NodeClassificationDataset):
    def __init__(self, dataset_name, *args, **kwargs):
        ...
        self.train_idx, self.valid_idx, self.test_idx = (
            split_idx["train"][self.category],
            split_idx["valid"][self.category],
            split_idx["test"][self.category],
        )
    ...
    def mag4HGT(self, hg):
        ...
        edges.update(
            {(v, e + "_inv", u): (dst, src) for (u, e, v), (src, dst) in edges.items()}
        )
        ...
        for ntype in hg.ntypes:
            if ntype != "paper":
                hg2.nodes[ntype].data["timestamp"] = th.zeros(
                    hg2.num_nodes(ntype), dtype=th.int64
                )

        # Aggregate bag-of-paper features
        hg2.nodes["paper"].data["h"] = hg.nodes["paper"].data["feat"]
        hg2.update_all(
            fn.copy_u("h", "m"), fn.mean("m", "h"), etype="has_topic"
        )  # field_of_study
        hg2.update_all(
            fn.copy_u("h", "m"), fn.mean("m", "h"), etype="writes_inv"
        )  # author
        hg2.update_all(
            fn.copy_u("h", "m"), fn.mean("m", "h"), etype="affiliated_with"
        )  # institution
        ...
        for ntype in hg2.ntypes:
            hg2.nodes[ntype].data["h"] = th.cat(
                [
                    ...
                    th.log10(hg2.nodes[ntype].data["deg"][:, None]),
                ],
                1,
            )
