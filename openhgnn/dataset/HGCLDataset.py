...
class HGCLDataset(DGLDataset):
    ...
    def __init__(self, name, raw_dir=None, force_reload=False, verbose=True):
        ...
        raw_dir = "./"
        url = (
            "https://s3.cn-north-1.amazonaws.com.cn/dgl-data/"
            + "dataset/{}.zip".format(name)
        )

        super(HGCLDataset, self).__init__(
            name=name,
            url=url,
            raw_dir=raw_dir,
    ...
    def create_graph(self):
        ...
        trainMat_i, trainMat_j, trainMat_data = (
            trainMat_coo.row,
            trainMat_coo.col,
            trainMat_coo.data,
        )
        ...
        userDistanceMat_i, userDistanceMat_j, userDistanceMat_data = (
            userDistanceMat_coo.row,
            userDistanceMat_coo.col,
            userDistanceMat_coo.data,
        )

        # itemMat
        itemMat_coo = itemMat.tocoo()
        itemMat_i, itemMat_j, itemMat_data = (
            itemMat_coo.row,
            itemMat_coo.col,
            itemMat_coo.data,
        )
        ...
        uiDistanceMat_i, uiDistanceMat_j, uiDistanceMat_data = (
            uiDistanceMat_coo.row,
            uiDistanceMat_coo.col,
            uiDistanceMat_coo.data,
        )

        graph_data = {
            ("user", "interact_train", "item"): (
                t.tensor(trainMat_i),
                t.tensor(trainMat_j),
            ),
            ("user", "distance", "user"): (
                t.tensor(userDistanceMat_i),
                t.tensor(userDistanceMat_j),
            ),
            ("item", "distance", "item"): (t.tensor(itemMat_i), t.tensor(itemMat_j)),
            ("user+item", "distance", "user+item"): (
                t.tensor(uiDistanceMat_i),
                t.tensor(uiDistanceMat_j),
            ),
            ("user", "interact_test", "item"): (
                t.tensor(testdata[:, 0]),
                t.tensor(testdata[:, 1]),
            ),
        }
