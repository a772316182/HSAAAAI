...
class lightGCN_Recommendation(RecommendationDataset):
    ...
    def download(self):
        prefix = (
            "https://raw.githubusercontent.com/gusye1234/LightGCN-PyTorch/master/data"
        )
        ...
        for filename in required_file:
            ...
            if not os.path.exists(file_path):
                try:
                    ...
                    print(
                        "If you don't have a vpn, please download the dataset from here: https://github.com/gusye1234/LightGCN-PyTorch"
                    )
                    print(
                        "\nAfter downloading the dataset, you need to store the files in the following path: "
                    )
                    print(
                        f"{os.getcwd()}\openhgnn\dataset\{self.dataset_name}\\train.txt"
                    )
                    print(
                        f"{os.getcwd()}\openhgnn\dataset\{self.dataset_name}\\test.txt"
                    )
                    exit()
...
class HINRecommendation(RecommendationDataset):
    ...
    def get_split(self, validation=True):
        ...
        test_graph = dgl.heterograph(
            {(self.user_name, self.target_link, self.item_name): test_edge},
        ...
        if validation:
            ...
            val_graph = dgl.heterograph(
                {(self.user_name, self.target_link, self.item_name): val_edge},
                {ntype: self.g.number_of_nodes(ntype) for ntype in self.out_ntypes},
            )

            train_graph = dgl.remove_edges(
                self.g, th.cat((val_index, test_index)), self.target_link
            )
            train_graph = dgl.remove_edges(
                train_graph, th.cat((val_index, test_index)), self.target_link_r
            )
        else:
...
class Test_Recommendation(RecommendationDataset):
    ...
    def preprocess(self):
        ...
        self.negative_g = negative_sampler(
            self.hg.to("cpu"),
...
class KGAT_recommendation(RecommendationDataset):
        ...
        for filename in required_file:
            ...
            if not os.path.exists(file_path):
                try:
                    ...
                    print(
                        "If you don't have a vpn, please download the dataset from here: https://github.com/xiangwang1223/knowledge_graph_attention_network/"
                    )
                    print(
                        "\nAfter downloading the dataset, you need to store the files in the following path: "
                    )
                    ...
                    print(
                        f"{os.getcwd()}\openhgnn\dataset\{self.dataset_name}\kg_final.txt"
                    )
                    print(
                        f"{os.getcwd()}\openhgnn\dataset\{self.dataset_name}\pretrain\mf.npz"
                    )
                    exit()
    ...
    def save(self):
        ...
        info_dict = {
            ...
            "n_cf_train": self.n_cf_train,
            "n_kg_train": self.n_kg_train,
        }
        os.makedirs(
            os.path.dirname(f"{self.processed_data_path}/other_info.pkl"), exist_ok=True
        )
