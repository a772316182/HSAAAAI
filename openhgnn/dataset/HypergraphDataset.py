...
class HgraphDataset(Dataset):
    ...
    def __init__(
        self, name, is_train=True, raw_dir=None, force_reload=False, verbose=True
    ):
    ...
    def process(self):
        if self.is_train:
            ...
            if not os.path.exists(f"./embeddings_{self.name}.pkl"):
                self.embeddings = generate_embeddings(self.edges, self.nums_type)
...
def generate_H(edge, nums_type):
    ...
    H = [
        csr_matrix(
            (np.ones(nums_examples), (edge[:, i], range(nums_examples))),
            shape=(nums_type[i], nums_examples),
        )
        for i in range(3)
    ]
    return H
...
def generate_embeddings(edge, nums_type):
    ...
    embeddings = [
        H[i].dot(s_vstack([H[j] for j in range(3) if j != i]).T).astype("float")
        for i in range(3)
    ]
    # 0-1 scaling
    for i in range(3):
