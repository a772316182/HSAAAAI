    ...
    for idx in eval_range:
        ...
        if filtered == "filtered":
            if preturb_side == "o":
                select_s = target_s
                select_o = filter(
                    triplets_to_filter, target_s, target_r, target_o, num_entities, "o"
                )
                target_idx = int((select_o == target_o).nonzero())
            elif preturb_side == "s":
                select_s = filter(
                    triplets_to_filter, target_s, target_r, target_o, num_entities, "s"
                )
                select_o = target_o
