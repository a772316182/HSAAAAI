...
def edata_in_out_mask(hg):
    ...
    for canonical_etype in hg.canonical_etypes:
        ...
        if canonical_etype[1][:4] == "rev-":
            hg.edges[canonical_etype].data["in_edges_mask"] = th.zeros(
                eid.shape[0], device=hg.device
            ).bool()
            hg.edges[canonical_etype].data["out_edges_mask"] = th.ones(
                eid.shape[0], device=hg.device
            ).bool()
        else:
            hg.edges[canonical_etype].data["out_edges_mask"] = th.zeros(
                eid.shape[0], device=hg.device
            ).bool()
            hg.edges[canonical_etype].data["in_edges_mask"] = th.ones(
                eid.shape[0], device=hg.device
            ).bool()
