...
from openhgnn.utils.dgl_graph import normalize_edges, edata_in_out_mask, load_link_pred
...
def run(model, g, config):
    ...
    for epoch in range(config.max_epoch):
        ...
        neg_edges, ns_samples = get_epoch_samples(
            g, epoch, config.dataset, config.num_ns_neg, config.device
        )
        ...
        ns_label = (
            th.cat([ns["label"] for ns in ns_samples])
            .type(th.float32)
            .to(config.device)
        )
...
def run_GTN(model, g, config):
    ...
    for i in range(config.max_epoch):
        ...
        print(
            "Train - Loss: {:.4f}, Macro_F1: {:.4f}, Micro_F1: {:.4f}".format(
                loss.item(), macro_f1, micro_f1
            )
        )
        optimizer.zero_grad()
        loss.backward()
        ...
        with th.no_grad():
            ...
            emd = h2dict(y_train, g.ndata["h"])
            tar_y = emd["paper"]
            loss, macro_f1, micro_f1 = cal_loss_f1(
                tar_y, tar_data, loss_func, "valid_mask"
            )
            print(
                "Valid - Loss: {:.4f}, Macro_F1: {:.4f}, Micro_F1: {:.4f}".format(
                    loss.item(), macro_f1, micro_f1
                )
            )
            val_f1 = macro_f1
            ...
            loss, macro_f1, micro_f1 = cal_loss_f1(
                tar_y, tar_data, loss_func, "test_mask"
            )
            print(
                "Test - Loss: {:.4f}, Macro_F1: {:.4f}, Micro_F1: {:.4f}".format(
                    loss.item(), macro_f1, micro_f1
                )
            )
            test_f1 = macro_f1
...
def run_CompGCN(model, hg, config):
    ...
    for epoch in range(config.max_epoch):
        ...
        val_loss = F.cross_entropy(logits[val_idx], labels[val_idx])
        val_acc = th.sum(logits[val_idx].argmax(dim=1) == labels[val_idx]).item() / len(
            val_idx
        )
        print(
            "Train Accuracy: {:.4f} | Train Loss: {:.4f} | Validation Accuracy: {:.4f} | Validation loss: {:.4f}".format(
...
def run_RGCN(model, hg, config):
    # optimizer
    optimizer = th.optim.Adam(
        model.parameters(), lr=config.lr, weight_decay=config.l2norm
    )
...
def run_HetGNN(model, hg, het_graph, config):
    ...
    print(
        "<Classification> DW    Micro-F1 = %.4f, Macro-F1 = %.4f" % (micro_f1, macro_f1)
    )
    # HetGNN Sampler
    from torch.utils.data import DataLoader
    from openhgnn.utils.sampler import (
        SkipGramBatchSampler,
        HetGNNCollator,
        NeighborSampler,
    )
    ...
    neighbor_sampler = NeighborSampler(
        het_graph, hg.ntypes, batch_sampler.num_nodes, config.device
    )
    collator = HetGNNCollator(neighbor_sampler, hg)
    dataloader = DataLoader(
        batch_sampler, collate_fn=collator.collate_train, num_workers=config.num_workers
    )
    ...
    micro_f1, macro_f1 = nc_with_split(
        x[config.category].to("cpu").detach(), labels, train_idx, test_idx
    )
    print(
        "<Classification>     Micro-F1 = %.4f, Macro-F1 = %.4f" % (micro_f1, macro_f1)
    )
    for i in range(config.max_epoch):
        model.train()
        ...
        print(
            "<Classification>     Micro-F1 = %.4f, Macro-F1 = %.4f"
            % (micro_f1, macro_f1)
        )
    pass
