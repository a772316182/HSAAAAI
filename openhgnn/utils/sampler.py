...
def get_node_data_loader(
    node_neighbors_min_num: int,
    num_layers: int,
    graph: dgl.DGLGraph,
    batch_size: int,
    sampled_node_type: str,
    train_idx: th.Tensor,
    valid_idx: th.Tensor,
    test_idx: th.Tensor,
    shuffle: bool = True,
    ...
    for layer in range(num_layers):
        sample_nodes_num.append(
            {etype: node_neighbors_min_num + layer for etype in graph.canonical_etypes}
        )
...
class MP2vecCollator(object):
    ...
    def collate_train(self, batches):
        ...
        pos_graph, neg_graph, block = self.construct_from_item_pairs(
    ...
    def randomwalk(self, length, walks, restart_prob):
        ...
        for i in range(self.g.number_of_nodes()):
            ...
            traces, types = dgl.sampling.random_walk(
                self.g, nodes, length=length, restart_prob=restart_prob
            )
        ...
        neighbor_graph = convert.heterograph(edge_dict, self.num_nodes)

        neighbor_graph = transform.to_simple(
            neighbor_graph, return_counts=self.weight_column
        )
...
class pro_sampler(_BaseNegativeSampler):
    ...
    def _generate(self, g, eids, canonical_etype):
        ...
        dst = np.random.choice(
            np.arange(0, g.number_of_nodes()), shape, replace=True, p=self.p
        )
...
def gen_neg_edges(g, num_neg, device):
    ...
    neg_edges = neg_sampler(
        g_homo, th.arange(0, g_homo.num_edges(), dtype=th.int64, device=device)
    )
...
def gen_ns_instances(g, num_ns_neg):
    ...
    def _sample_pos_ns(i, target_t):
        ...
        prev_sampled_types, etypes, current_sample_types = _get_current_sample_types(
            prev_sampled_types, pos_ns
        )
        prev_t = i
        while len(current_sample_types) > 0:
            # BFS
            for u_ntype, etype, v_ntype in zip(
                prev_sampled_types, etypes, current_sample_types
            ):
            ...
            prev_sampled_types, etypes, current_sample_types = (
                _get_current_sample_types(prev_sampled_types, pos_ns)
            )
        pos_ns["label"] = 1
        return pos_ns
...
def _get_ns_instance(g, epoch_seed, n_dataset, num_ns_neg):
    ...
    if os.path.exists(fname):
        ...
        # save
        data_to_save = {"ns_instances": ns_ins_list}
        with open(fname, "wb") as f:
            pickle.dump(data_to_save, f, protocol=pickle.HIGHEST_PROTOCOL)
