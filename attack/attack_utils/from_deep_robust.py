...
def select_target_nodes(target_gcn, labels, idx_test):
    """
    ...
    idx_test = torch.squeeze(torch.nonzero(idx_test * 1)).cpu().numpy().tolist()
...
def to_tensor(adj, features, labels=None, device="cpu"):
    ...
    if sp.issparse(features):
        ...
        features = torch.FloatTensor(np.array(features.detach().cpu().numpy()))
...
def normalize_adj_tensor(adj, sparse=False):
    ...
    if sparse:
        ...
        mx = torch.eye(adj.shape[0]).to(device) + adj
        rowsum = mx.sum(1)
...
def loss_acc(output, labels, targets, avg_loss=True):
    ...
    loss = F.nll_loss(
        output[targets], labels[targets], reduction="mean" if avg_loss else "none"
    )
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    return torch.sparse.FloatTensor(
        sparseconcat.t(), sparsedata, torch.Size(sparse_mx.shape)
    )
...
def get_splits_each_class(labels, train_size):
    ...
    for i in range(nclass):
        ...
        idx_val = np.hstack((idx_val, labels_i[train_size : 2 * train_size])).astype(
            np.int
        )
        idx_test = np.hstack((idx_test, labels_i[2 * train_size :])).astype(np.int)

    return (
        np.random.permutation(idx_train),
        np.random.permutation(idx_val),
...
def likelihood_ratio_filter(
    node_pairs, modified_adjacency, original_adjacency, d_min, threshold=0.004
):
    """
    ...
    ll_orig, alpha_orig, n_orig, sum_log_degrees_original = (
        degree_sequence_log_likelihood(original_degree_sequence, d_min)
    )
    ll_current, alpha_current, n_current, sum_log_degrees_current = (
        degree_sequence_log_likelihood(current_degree_sequence, d_min)
    )

    ll_comb, alpha_comb, n_comb, sum_log_degrees_combined = (
        degree_sequence_log_likelihood(concat_degree_sequence, d_min)
    )
    ...
    new_lls, new_alphas, new_ns, new_sum_log_degrees = (
        updated_log_likelihood_for_edge_changes(node_pairs, modified_adjacency, d_min)
    )
